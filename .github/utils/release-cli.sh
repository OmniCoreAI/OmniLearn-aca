...
# OmniLearn CLI release script. Bumps the CLI version on dev, tags `cli-<version>`
...
if [ -z "$VERSION" ]; then
  echo ""
  echo "  📦 OmniLearn CLI Release Script"
  echo ""
...
echo ""
echo "  🚀 OmniLearn CLI Release — ${VERSION}"
echo "  ─────────────────────────────────────"
echo ""
...
PUBLISHED="$(npm view omnilearn@"$VERSION" version 2>/dev/null || true)"
[ -n "$PUBLISHED" ] && die "omnilearn@$VERSION is already published on npm. Pick a new version."
