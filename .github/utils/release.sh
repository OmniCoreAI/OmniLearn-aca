...
CHANGELOG_FILE=$(mktemp)
{
  ...
  echo "npx omnilearn backup"
  echo ""
  echo "# Update to this specific version"
  echo "npx omnilearn update --version ${VERSION}"
  echo "\`\`\`"
  echo ""
  echo "**Docker image:**"
  echo ""
  echo "\`\`\`bash"
  echo "docker pull ghcr.io/omnilearn/app:${VERSION}"
  echo "\`\`\`"
  echo ""
