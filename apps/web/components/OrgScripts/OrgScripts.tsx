...
const OrgScripts: React.FC = () => {
  ...
  const loadScript = (scriptContent: string, scriptName: string) => {
    try {
      ...
      const safeScriptId = `omnilearn-org-script-${scriptName.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-${Math.random().toString(36).substr(2, 9)}`
      ...
      if (scriptContent.trim().toLowerCase().startsWith('<script')) {
        ...
        if (scriptTag.src) {
          ...
          scriptElement.textContent = `
            /* OmniLearn Organization Script - ${scriptName} */
            try {
              (function() {
                'use strict';
                ${sanitizedContent}
        ...
        const comment = document.createComment(` OmniLearn Organization Script - ${scriptName} (${safeScriptId}) `)
  ...
  useEffect(() => {
    ...
    return () => {
      const scripts = document.querySelectorAll('script[id^="omnilearn-org-script-"]')
