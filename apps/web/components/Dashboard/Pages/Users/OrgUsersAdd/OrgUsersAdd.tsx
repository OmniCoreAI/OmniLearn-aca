...
import { adminCreateUser, inviteBatchUsers, removeInvitedUser } from '@services/organizations/invites'
...
// Card that lets an admin provision an account directly (no email round-trip).
// The backend returns a one-time temporary password that we surface here for
// the admin to copy; the new user is forced to change it on first login.
      ...
      <div className="flex flex-wrap gap-3 items-start justify-between px-4 sm:px-6 py-5 border-b border-gray-100">
        <div className="flex-1">
          <h1 className="font-bold text-xl text-gray-800">
            {t('dashboard.users.create_account.title', 'Create an account directly')}
          </h1>
          <p className="text-sm text-gray-500 mt-0.5">
            {t(
              'dashboard.users.create_account.subtitle',
              'Provision a user now with a temporary password — no email required. They must change it on first login.'
            )}
          </p>
        </div>
      </div>
        ...
        <form onSubmit={handleCreate} className="px-6 py-5 space-y-4">
          ...
          <div className="sm:max-w-[50%]">
            ...
            <select className={inputCls} value={roleUuid} onChange={(e) => setRoleUuid(e.target.value)}>
              <option value="">{t('dashboard.users.create_account.default_role', 'Default (Learner)')}</option>
              {(roles || []).map((role: any) => (
          ...
          <div className="flex justify-end">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-black hover:bg-black/90 disabled:opacity-40 disabled:cursor-not-allowed rounded-lg font-semibold text-sm text-white transition-all"
            >
...
function OrgUsersAdd() {
  ...
  return (
    <>
      ...
      {/* Create account directly (admin-provisioned, temp password) */}
      <AdminCreateUserCard />
      <div className="h-6"></div>
