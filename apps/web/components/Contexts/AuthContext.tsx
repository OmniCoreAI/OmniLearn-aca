...
import {
  getAPIUrl,
  getOMNILEARN_TOP_DOMAIN_VAL,
  getOMNILEARN_DOMAIN_VAL,
...
const AUTH_BROADCAST_CHANNEL = 'omnilearn_auth_sync'
const OAUTH_STATE_COOKIE = 'OL_oauth_state'
...
function isCustomDomain(): boolean {
  ...
  const domain = getOMNILEARN_DOMAIN_VAL()
...
function clearSessionMarker(): void {
  ...
  document.cookie = `OL_session=; path=/${sameSiteAttr}${secureAttr}${domainAttr}; ${expired}`
  document.cookie = `OL_session=; path=/${sameSiteAttr}${secureAttr}; ${expired}`
}
...
export function SessionProvider({
  ...
  const hasSessionMarker = useCallback((): boolean => {
    return typeof document !== 'undefined' && document.cookie.includes('OL_session')
  }, [])
      ...
      try {
        ...
        if (provider === 'google') {
          ...
          if (options.orgSlug || options.orgId) {
            if (options.orgSlug) {
              document.cookie = `OL_oauth_orgslug=${options.orgSlug}${baseAttributes}${domainAttr}`
            }
            if (options.orgId) {
              document.cookie = `OL_oauth_org_id=${options.orgId}${baseAttributes}${domainAttr}`
            }
          }
          ...
          const redirectUri = `${window.location.protocol}//${getOMNILEARN_DOMAIN_VAL()}/auth/callback/google`
  ...
  const handleSignOut = useCallback(async (options: SignOutOptions = {}) => {
    ...
    document.cookie = `OL_oauth_orgslug=; path=/${expireAttr}${secureAttr}${domainAttr}`
    document.cookie = `OL_oauth_org_id=; path=/${expireAttr}${secureAttr}${domainAttr}`
