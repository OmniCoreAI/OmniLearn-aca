...
import { getOMNILEARN_DOMAIN_VAL, getOMNILEARN_HTTP_PROTOCOL_VAL } from '@services/config/config'
...
function EmbedActivityClient({ activityId, courseuuid, orgslug, bgcolor }: EmbedActivityClientProps) {
  ...
  const showOmniLearnLogo = searchParams.get('showomnilearnlogo') !== 'false'
  ...
  const getActivityUrl = () => {
    ...
    // (e.g. app.omnilearn.io), so a relative path would resolve to the wrong host.
    if (typeof window !== 'undefined' && orgslug) {
      const domain = getOMNILEARN_DOMAIN_VAL()
      const protocol = getOMNILEARN_HTTP_PROTOCOL_VAL()
  ...
  if (!isEmbeddable) {
    return (
      <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-8">
        ...
        {showOmniLearnLogo && <PoweredByBadge activityUrl={getActivityUrl()} />}
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen relative" style={customStyles}>
      ...
      {showOmniLearnLogo && ready && <PoweredByBadge activityUrl={getActivityUrl()} />}
    </div>
  )
}
