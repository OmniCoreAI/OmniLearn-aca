...
const BACKEND_URL = (getConfig('NEXT_PUBLIC_OMNILEARN_BACKEND_URL') || 'http://localhost:1338').replace(/\/+$/, '')
...
function clearAuthCookies(response: NextResponse, request: NextRequest) {
  ...
  if (domain) {
    ...
    response.headers.append('Set-Cookie', `OL_session=; Path=/; Domain=${domain}; Max-Age=0; SameSite=Lax${securePart}`)
  }
  ...
  response.headers.append('Set-Cookie', `OL_session=; Path=/; Max-Age=0; SameSite=Lax${securePart}`)
}
