...
//
//   1. multi (EE-only):   slug.{OMNILEARN_DOMAIN} subdomain detection +
...
function setInstanceCookies(response: NextResponse, info: InstanceInfo) {
  response.cookies.set({ name: 'OL_tenancy', value: info.tenancy, path: '/' })
  response.cookies.set({ name: 'OL_default_org', value: info.default_org_slug, path: '/' })
  response.cookies.set({ name: 'OL_frontend_domain', value: info.frontend_domain, path: '/' })
  response.cookies.set({ name: 'OL_top_domain', value: info.top_domain, path: '/' })
  response.cookies.set({ name: 'OL_mode', value: info.mode, path: '/' })
  return response
}
