...
const getOMNILEARN_HTTP_PROTOCOL = () =>
  (getConfig('NEXT_PUBLIC_OMNILEARN_HTTPS') === 'true') ? 'https://' : 'http://'
const getOMNILEARN_BACKEND_URL = () => getConfig('NEXT_PUBLIC_OMNILEARN_BACKEND_URL', 'http://localhost/')
const getOMNILEARN_DOMAIN = () => {
  // 1. Env var (backward compat for existing deploys)
  const envVal = getConfig('NEXT_PUBLIC_OMNILEARN_DOMAIN')
  if (envVal) return envVal
  // 2. Cookie set by middleware from backend instance info
  const cookieVal = getCookieValue('OL_frontend_domain')
  if (cookieVal) return cookieVal
  // 3. Default
  return 'localhost'
}
const getOMNILEARN_TOP_DOMAIN = () => {
  // 1. Env var (backward compat for existing deploys)
  const envVal = getConfig('NEXT_PUBLIC_OMNILEARN_TOP_DOMAIN')
  if (envVal) return envVal
  // 2. Cookie set by middleware from backend instance info
  const cookieVal = getCookieValue('OL_top_domain')
  if (cookieVal) return cookieVal
  // 3. Derive from DOMAIN by stripping port
  const domain = getOMNILEARN_DOMAIN()
...
const getOMNILEARN_PLATFORM_URL = (): string | null => {
  // NEXT_PUBLIC_ variant (available client-side via runtime config)
  const pubVal = getConfig('NEXT_PUBLIC_OMNILEARN_PLATFORM_URL')
  if (pubVal) return pubVal.replace(/\/+$/, '')
  // Non-prefixed variant (server-side only, backward compat)
  const val = getConfig('OMNILEARN_PLATFORM_URL')
  if (val) return val.replace(/\/+$/, '')
  return null
}

// Export getter functions for dynamic runtime configuration
export const getOMNILEARN_HTTP_PROTOCOL_VAL = getOMNILEARN_HTTP_PROTOCOL
export const getOMNILEARN_BACKEND_URL_VAL = getOMNILEARN_BACKEND_URL
export const getOMNILEARN_DOMAIN_VAL = getOMNILEARN_DOMAIN
export const getOMNILEARN_TOP_DOMAIN_VAL = getOMNILEARN_TOP_DOMAIN
export const getPOSTHOG_KEY_VAL = getPOSTHOG_KEY
export const getOMNILEARN_PLATFORM_URL_VAL = getOMNILEARN_PLATFORM_URL
...
export const OMNILEARN_HTTP_PROTOCOL = getOMNILEARN_HTTP_PROTOCOL()
export const OMNILEARN_BACKEND_URL = getOMNILEARN_BACKEND_URL()
export const OMNILEARN_DOMAIN = getOMNILEARN_DOMAIN()
export const OMNILEARN_TOP_DOMAIN = getOMNILEARN_TOP_DOMAIN()
...
// Derive API URL from backend URL (with backward compat for NEXT_PUBLIC_OMNILEARN_API_URL)
const deriveAPIUrl = (): string => {
  // Backward compat: if explicit API URL is set, use it
  const explicitApiUrl = getConfig('NEXT_PUBLIC_OMNILEARN_API_URL')
  if (explicitApiUrl) return explicitApiUrl
  // Derive from backend URL
  const backendUrl = getOMNILEARN_BACKEND_URL().replace(/\/+$/, '')
...
export const getBackendUrl = () => getOMNILEARN_BACKEND_URL()

/**
 * Get the upgrade/plan URL for a given org.
 * Uses OMNILEARN_PLATFORM_URL (the main platform, e.g. omnilearn.app).
...
export const getUpgradeUrl = (orgSlug: string): string | null => {
  ...
  const platformUrl = getOMNILEARN_PLATFORM_URL()
  if (!platformUrl) return null
  return `${platformUrl}/dashboard/${orgSlug}/plan`
}

/**
 * Build a URL on the platform domain (e.g. omnilearn.app).
...
//
// Reads the `OL_tenancy` cookie set by the middleware on every request. The
...
//
// We deliberately do NOT consult `NEXT_PUBLIC_OMNILEARN_MULTI_ORG` here —
...
export const getTenancy = (): TenancyMode => {
  const cookieVal = getCookieValue('OL_tenancy')
...
export const getCustomDomainFromContext = (): string | null => {
  if (typeof window !== 'undefined') {
    ...
    // Check if current hostname is a custom domain (not a subdomain of OMNILEARN_DOMAIN)
    ...
    try {
      ...
      for (const cookie of cookies) {
        ...
        if (name === 'OL_custom_domain' && value) {
...
export const getUriWithOrg = (orgslug: string, path: string) => {
  ...
  if (typeof window !== 'undefined') {
    ...
    const baseDomain = stripPort(getOMNILEARN_DOMAIN())
  ...
  if (orgslug) {
    const protocol = getOMNILEARN_HTTP_PROTOCOL()
    const domain = getOMNILEARN_DOMAIN()
    return `${protocol}${orgslug}.${domain}${path}`
  }
  const explicitDomain = getConfig('NEXT_PUBLIC_OMNILEARN_DOMAIN')
  if (explicitDomain) {
    const protocol = getOMNILEARN_HTTP_PROTOCOL()
...
/**
 * Get the current deployment mode from the OL_mode cookie set by middleware.
...
export const getDeploymentMode = (): DeploymentMode => {
  return (getCookieValue('OL_mode') as DeploymentMode) || 'oss'
}
...
export const getDefaultOrg = () => {
  ...
  const envVal = getConfig('NEXT_PUBLIC_OMNILEARN_DEFAULT_ORG')
  if (envVal) return envVal
  // 2. Client-side: read cookie set by middleware
  const cookieVal = getCookieValue('OL_default_org')
  if (cookieVal) return cookieVal
  // 3. Default
  return 'default'
}
