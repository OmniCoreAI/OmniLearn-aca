 ...
 *
 * The middleware writes `OL_tenancy` ('multi' | 'single') on every request.
...
async function getServerTenancy(): Promise<'multi' | 'single'> {
  try {
    ...
    const t = cookieStore.get('OL_tenancy')?.value
 ...
 * Priority: subdomain (multi only) > OL_org cookie.
 */
export async function getOrgSlug(): Promise<string | null> {
  ...
  const cookieStore = await cookies()
  const orgslugCookie = cookieStore.get('OL_org')
...
async function getOrgSlugFromSubdomainViaEE(): Promise<string | null> {
  try {
    ...
    const frontendDomain =
      process.env.NEXT_PUBLIC_OMNILEARN_DOMAIN
      || cookieStore.get('OL_frontend_domain')?.value
      || 'localhost'
