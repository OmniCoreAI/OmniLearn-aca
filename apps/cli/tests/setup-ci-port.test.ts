...
describe('setup --ci automatic port fallback', () => {
  ...
  it('falls back to an available port when 80 is taken', async () => {
    ...
    const cfg = JSON.parse(fs.readFileSync(path.join(home, '.omnilearn', 'fb', 'omnilearn.config.json'), 'utf-8'))
  ...
  it('accepts an explicitly-requested port that is free', async () => {
    ...
    const cfg = JSON.parse(fs.readFileSync(path.join(home, '.omnilearn', 'okport', 'omnilearn.config.json'), 'utf-8'))
