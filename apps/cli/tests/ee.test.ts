...
describe('generateEeDockerCompose (single)', () => {
  ...
  it('uses the EE registry images', () => {
    expect(yml).toContain('images.omnilearn.app/enterprise-backend:${EE_IMAGE_TAG:-prod}')
    expect(yml).toContain('images.omnilearn.app/enterprise-frontend:${EE_IMAGE_TAG:-prod}')
    expect(yml).toContain('images.omnilearn.app/enterprise-collab:${EE_IMAGE_TAG:-prod}')
  })
  it('namespaces project by deploymentId', () => {
    expect(yml).toContain('name: omnilearn-abcd1234')
  })
  ...
  it('is single-tenant', () => {
    expect(yml).toContain('OMNILEARN_TENANCY: single')
    expect(yml).toContain('NEXT_PUBLIC_OMNILEARN_MULTI_ORG: "false"')
    expect(yml).not.toContain('OMNILEARN_TENANCY: multi')
  })
...
describe('generateEeDockerCompose (agency)', () => {
  ...
  it('is multi-tenant', () => {
    expect(yml).toContain('OMNILEARN_TENANCY: multi')
    expect(yml).toContain('NEXT_PUBLIC_OMNILEARN_MULTI_ORG: "true"')
  })
  it('uses AGENCY_DOMAIN + cookie domain', () => {
    expect(yml).toContain('AGENCY_DOMAIN: ${AGENCY_DOMAIN:?AGENCY_DOMAIN is required}')
    expect(yml).toContain('OMNILEARN_COOKIE_DOMAIN: .${AGENCY_DOMAIN}')
  })
})
...
describe('generateEeEnv', () => {
  ...
  it('single: writes the env-var contract', () => {
    ...
    expect(env).toContain('OMNILEARN_LICENSE_KEY=lh_live_TESTKEY')
    ...
    expect(env).toContain('OMNILEARN_AUTH_JWT_SECRET_KEY=JWTSECRET')
    expect(env).toContain('COLLAB_INTERNAL_KEY=COLLABKEY')
    expect(env).toContain('OMNILEARN_INITIAL_ADMIN_EMAIL=admin@acme.com')
    expect(env).toContain('OMNILEARN_INITIAL_ADMIN_PASSWORD=sup3rsecret')
  })
  ...
  it('quotes special-char admin password literally (single-quote scheme)', () => {
    ...
    expect(env).toContain("OMNILEARN_INITIAL_ADMIN_PASSWORD='p@ss$word#1'")
  })
  it('quotes admin email so $ cannot interpolate', () => {
    const env = generateEeEnv(baseEe({ adminEmail: 'a$b@corp.com' }), secrets)
    expect(env).toContain("OMNILEARN_INITIAL_ADMIN_EMAIL='a$b@corp.com'")
  })
})
...
describe('writeConfig persists EE metadata', () => {
  ...
  it('stores edition/tenancy/localTls for enterprise', () => {
    ...
    const json = JSON.parse(fs.readFileSync(path.join(dir, 'omnilearn.config.json'), 'utf-8'))
...
describe('setup --ci --edition enterprise (file generation)', () => {
  ...
  it('single: generates the full EE file set', () => {
    ...
    expect(compose).toContain('OMNILEARN_TENANCY: single')
  })
  ...
  it('agency + local-tls: multi-tenant compose + override + local_certs', () => {
    ...
    const compose = fs.readFileSync(path.join(dir, 'docker-compose.yml'), 'utf-8')
    expect(compose).toContain('OMNILEARN_TENANCY: multi')
  ...
  it('reuses deploymentId on redeploy (so data volumes are not orphaned)', () => {
    cli(base(''))
    const id1 = JSON.parse(fs.readFileSync(path.join(dir, 'omnilearn.config.json'), 'utf-8')).deploymentId
    const composeName1 = fs.readFileSync(path.join(dir, 'docker-compose.yml'), 'utf-8').match(/^name:\s*(\S+)/m)?.[1]
    cli(base(''))
    const id2 = JSON.parse(fs.readFileSync(path.join(dir, 'omnilearn.config.json'), 'utf-8')).deploymentId
