  ...
  return {
    ...
    execSync: vi.fn((cmd: string) => {
      if (typeof cmd === 'string' && cmd.includes('docker ps')) {
        return Buffer.from('omnilearn-app-dep1\tUp 2 hours\tghcr.io/omnilearn/app:1.4.2\n')
      }
...
describe('doctor — DNS resolution check', () => {
  ...
  beforeEach(() => {
    ...
    fs.writeFileSync(path.join(installDir, '.env'), 'OMNILEARN_DOMAIN=learn.example.com\n')
