...
describe('scale — mem_limit parse/set', () => {
  const compose = [
    ...
    '    container_name: omnilearn-app-dep1',
    '    mem_limit: 2g',
    '  db:',
    '    image: pgvector/pgvector:pg16',
    '    container_name: omnilearn-db-dep1',
    '  redis:',
    '    image: redis:7-alpine',
    '    container_name: omnilearn-redis-dep1',
    '',
  ].join('\n')
  ...
  it('setMemLimit round-trips for every standard service', () => {
    let c = compose
    c = setMemLimit(c, 'omnilearn-app', '4g')
    ...
    expect(limits.get('omnilearn-app')).toBe('4g')
...
describe('checkDevEnv', () => {
  ...
  it('returns true when every required dev var is present (quoted/commented values too)', async () => {
    ...
    writeEnv('apps/api/.env',
      'OMNILEARN_AUTH_JWT_SECRET_KEY="jwt-secret"  # the signing key\nCOLLAB_INTERNAL_KEY=\'collab-key\'\n')
...
describe('interactive command flows', () => {
  ...
  beforeEach(async () => {
    ...
    installDir = path.join(home, '.omnilearn', 'test')
    fs.mkdirSync(installDir, { recursive: true })
    fs.writeFileSync(path.join(installDir, 'omnilearn.config.json'), JSON.stringify({
    ...
    fs.writeFileSync(path.join(installDir, '.env'), 'OMNILEARN_DOMAIN=localhost\nHTTP_PORT=8080\n')
  ...
  it('scale shows fallback stats when compose stats fails', async () => {
    ...
    m.mockImplementation(((cmd: string) => {
      ...
      if (cmd.includes('docker ps')) return Buffer.from('omnilearn-app-dep1\tUp 2 hours\tghcr.io/omnilearn/app:1.4.2\n')
      if (cmd.includes('docker stats')) return Buffer.from('NAME\tCPU\nomnilearn-app-dep1\t5%\n')
      return Buffer.from('')
    }) as never)
    H.q.text.push('512m', '1g', '256m')
  ...
  it('deployments → view renders a per-deployment container status table', async () => {
    ...
    m.mockImplementation(((cmd: string) => {
      if (cmd.includes('docker ps')) {
        return Buffer.from([
          'omnilearn-app-abc123\tUp 2 hours\tomnilearn/app:1.4.8',
          'omnilearn-db-abc123\tUp 2 hours\tpostgres:16',
          'omnilearn-redis-abc123\tExited (0) 1 min ago\tredis:7',
        ].join('\n'))
      }
      return Buffer.from('')
    }) as never)
  ...
  it('deployments → scale warns when stats lookup throws in the fallback path', async () => {
    ...
    m.mockImplementation(((cmd: string) => {
      ...
      if (cmd.includes('docker ps')) {
        ...
        return Buffer.from('omnilearn-app-dep1\tUp 2 hours\tomnilearn/app:1.4.8')
      }
      return Buffer.from('')
    }) as never)
  ...
  it('deployments → view falls back to the app-only listing when the broad query fails', async () => {
    ...
    let ps = 0
    m.mockImplementation(((cmd: string) => {
      if (cmd.includes('name=omnilearn-app-')) {
        return Buffer.from('omnilearn-app-abc123\tUp 2 hours\tomnilearn/app:1.4.8')
      }
      if (cmd.includes('name=omnilearn-')) { ps++; throw new Error('broad query failed') }
      return Buffer.from('')
    }) as never)
    H.q.select.push('view')
    await expect(deploymentsCommand()).resolves.toBeUndefined()
  ...
  it('scale skips an invalid limit and leaves that service unchanged', async () => {
    ...
    const limits = parseMemLimit(path.join(installDir, 'docker-compose.yml'))
    expect(limits.has('omnilearn-app')).toBe(false)
    expect(limits.get('db')).toBe('1g')
    expect(limits.has('redis')).toBe(false)
  })

  it('env edits a variable and persists it to .env', async () => {
    H.q.select.push('domain', 'OMNILEARN_DOMAIN', '_done') // category, key, then done
    ...
    expect(fs.readFileSync(path.join(installDir, '.env'), 'utf-8'))
      .toContain('OMNILEARN_DOMAIN=school.example.com')
  })

  it('env makes no change when the editor is dismissed immediately', async () => {
    H.q.select.push('_done')
    await envCommand()
    expect(fs.readFileSync(path.join(installDir, '.env'), 'utf-8')).toContain('OMNILEARN_DOMAIN=localhost')
  })
  ...
  it('deployments → scale sets mem limits through the resource menu', async () => {
    ...
    await expect(deploymentsCommand()).resolves.toBeUndefined()
    expect(parseMemLimit(path.join(installDir, 'docker-compose.yml')).get('omnilearn-app')).toBe('512m')
  })
  ...
  it('deployments → scale replaces an existing mem_limit', async () => {
    fs.writeFileSync(path.join(installDir, 'docker-compose.yml'), [
      'name: omnilearn-dep1', 'services:',
      '  omnilearn-app:', '    container_name: omnilearn-app-dep1', '    mem_limit: 1g',
      '  db:', '    container_name: omnilearn-db-dep1',
      '  redis:', '    container_name: omnilearn-redis-dep1', '',
    ].join('\n'))
    H.q.select.push('scale')
  ...
  it('env masks existing secrets and preserves unrelated/comment lines on write', async () => {
    fs.writeFileSync(path.join(installDir, '.env'), [
      ...
      'POSTGRES_DB=omnilearn',              // not edited → passed through unchanged (105)
      '',
    ].join('\n'))
    ...
    expect(body).toContain('POSTGRES_DB=omnilearn')            // 105
  ...
  it('env exits when the install has no .env file', async () => {
    ...
    fs.writeFileSync(path.join(proj, 'omnilearn.config.json'), JSON.stringify({
      version: '1.4.8', deploymentId: 'dep1', createdAt: '2026-01-01T00:00:00Z',
      installDir: proj, domain: 'localhost', httpPort: 8080,
      useHttps: false, autoSsl: false, useExternalDb: false, orgSlug: 'default',
    }))
    fs.writeFileSync(path.join(proj, 'docker-compose.yml'), 'name: omnilearn-dep1\nservices:\n  omnilearn-app:\n')
...
describe('dev command guards', () => {
  ...
  it('exits when not inside a OmniLearn project', async () => {
...
describe('command success paths', () => {
  ...
  beforeEach(async () => {
    ...
    fs.writeFileSync(path.join(installDir, '.env'), 'OMNILEARN_DOMAIN=localhost\nHTTP_PORT=8080\n')
    fs.writeFileSync(path.join(installDir, 'docker-compose.yml'), 'name: omnilearn-dep1\nservices:\n  omnilearn-app:\n    container_name: omnilearn-app-dep1\n')
    origHome = process.env.HOME
    process.env.HOME = home
    H.reset()
    vi.spyOn(process, 'exit').mockImplementation(((code?: number) => {
  ...
  it('health reports PostgreSQL not ready when pg_isready fails', async () => {
    const m = (await import('node:child_process')).execSync as unknown as ReturnType<typeof vi.fn>
    m.mockImplementation(((cmd: string) => {
      if (cmd.includes('docker ps')) return Buffer.from('omnilearn-db-dep1\tUp 2 hours\tpgvector/pgvector:pg16\n')
      if (cmd.includes('State.Running')) return Buffer.from('true')
  ...
  it('health notes when there are no running containers for resource stats', async () => {
    const m = (await import('node:child_process')).execSync as unknown as ReturnType<typeof vi.fn>
    m.mockImplementation(((cmd: string) => {
      if (cmd.includes('docker ps')) return Buffer.from('omnilearn-app-dep1\tExited (0) 1 min ago\tghcr.io/omnilearn/app:1.4.2\n')
  ...
  it('doctor and health walk the green path when containers report healthy', async () => {
    ...
    m.mockImplementation(((cmd: string) => {
      if (cmd.includes('docker ps')) return Buffer.from(
        ...
        'omnilearn-redis-dep1\tUp 2 hours\tredis:7-alpine\n')
      if (cmd.includes('State.Running')) return Buffer.from('true')
  ...
  it('doctor reports the configured port as in use when occupied', async () => {
    ...
    m.mockImplementation(((cmd: string) =>
      cmd.includes('docker ps')
        ? Buffer.from('omnilearn-app-dep1\tUp 2 hours\tghcr.io/omnilearn/app:1.4.2\n')
        : cmd.includes('State.Running') ? Buffer.from('true') : Buffer.from('')) as never)
    try {
  ...
  it('doctor warns on short secrets and unreadable logs', async () => {
    fs.writeFileSync(path.join(installDir, '.env'),
      'OMNILEARN_DOMAIN=localhost\nHTTP_PORT=8080\nOMNILEARN_AUTH_JWT_SECRET_KEY=abc\n') // too short
    const m = (await import('node:child_process')).execSync as unknown as ReturnType<typeof vi.fn>
    m.mockImplementation(((cmd: string) => {
      if (cmd.includes('docker ps')) return Buffer.from('omnilearn-app-dep1\tUp 2 hours\tghcr.io/omnilearn/app:1.4.2\n')
      if (cmd.includes('State.Running')) return Buffer.from('true')
  ...
  it('doctor flags restarting/stopped containers and high restart counts', async () => {
    ...
    m.mockImplementation(((cmd: string) => {
      if (cmd.includes('docker ps')) return Buffer.from(
        'omnilearn-app-dep1\tRestarting (1) 3 seconds ago\tghcr.io/omnilearn/app:1.4.2\n' +
        'omnilearn-db-dep1\tExited (1) 5 minutes ago\tpgvector/pgvector:pg16\n' +
        'omnilearn-redis-dep1\tUp 2 hours\tredis:7-alpine\n')
      if (cmd.includes('State.Running')) return Buffer.from('false')
  ...
  it('doctor confirms when every required env var is present and strong', async () => {
    fs.writeFileSync(path.join(installDir, '.env'), [
      'OMNILEARN_DOMAIN=localhost',
      'OMNILEARN_SQL_CONNECTION_STRING=postgresql://u:p@db:5432/lh',
      'OMNILEARN_REDIS_CONNECTION_STRING=redis://redis:6379/0',
      'OMNILEARN_AUTH_JWT_SECRET_KEY=a-strong-jwt-secret-value',
  ...
  it('doctor warns on low disk space (sub-1G)', async () => {
    ...
    m.mockImplementation(((cmd: string) =>
      cmd.includes('df -h') ? Buffer.from('0.5g\n')
        : cmd.includes('docker ps') ? Buffer.from('omnilearn-app-dep1\tUp 2 hours\tghcr.io/omnilearn/app:1.4.2\n')
          : cmd.includes('State.Running') ? Buffer.from('true') : Buffer.from('')) as never)
    await expect(doctorCommand()).resolves.toBeUndefined()
  })
  ...
  it('shell opens an interactive session in a running container', async () => {
    ...
    m.mockImplementation(((cmd: string) =>
      cmd.includes('docker ps')
        ? Buffer.from('omnilearn-app-dep1\tUp 2 hours\tghcr.io/omnilearn/app:1.4.2\n')
        : Buffer.from('')) as never)
    H.q.select.push('omnilearn-app-dep1')
  ...
  it('shell/logs exit when the config has no deployment id and none is detected', async () => {
    ...
    fs.writeFileSync(path.join(cwdDir, 'omnilearn.config.json'), JSON.stringify({
      version: '1.4.8', createdAt: '2026-01-01T00:00:00Z', // no deploymentId
    ...
    process.env.HOME = emptyHome // no ~/.omnilearn installs → findInstallDir falls back to cwd
  ...
  it('shell exits when no containers are running', async () => {
    ...
    m.mockImplementation(((cmd: string) =>
      cmd.includes('docker ps')
        ? Buffer.from('omnilearn-app-dep1\tExited (0)\tghcr.io/omnilearn/app:1.4.2\n') // not "Up"
        : Buffer.from('')) as never)
    await expect(shellCommand()).rejects.toBeInstanceOf(ProcessExit)
  })
  ...
  it('logs exits when no containers are running', async () => {
    ...
    m.mockImplementation(((cmd: string) => {
      ...
      if (cmd.includes('docker ps')) return Buffer.from('omnilearn-app-dep1\tExited (0)\tghcr.io/omnilearn/app:1.4.2\n') // not "Up"
      return Buffer.from('')
    }) as never)
    await expect(logsCommand()).rejects.toBeInstanceOf(ProcessExit)
  })
...
describe('setup / update in-process', () => {
  ...
  it('setup --ci --no-start generates a complete install', async () => {
    ...
    const dir = path.join(home, '.omnilearn', 'unit')
    expect(fs.existsSync(path.join(dir, 'docker-compose.yml'))).toBe(true)
    expect(fs.existsSync(path.join(dir, '.env'))).toBe(true)
    expect(fs.existsSync(path.join(dir, 'omnilearn.config.json'))).toBe(true)
    expect(fs.readFileSync(path.join(dir, '.env'), 'utf-8'))
      .toContain('OMNILEARN_INITIAL_ADMIN_EMAIL=admin@school.dev')
  })
  ...
  it('the interactive wizard generates a complete community install', async () => {
    ...
    const dir = path.join(home, '.omnilearn', 'default')
    expect(fs.existsSync(path.join(dir, 'docker-compose.yml'))).toBe(true)
    expect(fs.existsSync(path.join(dir, '.env'))).toBe(true)
    expect(fs.existsSync(path.join(dir, 'omnilearn.config.json'))).toBe(true)
    expect(fs.readFileSync(path.join(dir, '.env'), 'utf-8'))
  ...
  it('the interactive wizard lets you go back and edit a step before confirming', async () => {
    ...
    const cfg = JSON.parse(fs.readFileSync(
      path.join(home, '.omnilearn', 'edited-install', 'omnilearn.config.json'), 'utf-8'))
  ...
  it('the wizard edit menu can re-run the install-directory step', async () => {
    ...
    await setupCommand({})
    expect(fs.existsSync(path.join(home, '.omnilearn', 'edit-dir', 'docker-compose.yml'))).toBe(true)
  })

  it('the wizard edit menu can re-run the domain step', async () => {
    wizardThenEdit('edit-dom', 1, { text: ['localhost', '8200'] }) // case 1: domain + port
    await setupCommand({})
    const cfg = JSON.parse(fs.readFileSync(path.join(home, '.omnilearn', 'edit-dom', 'omnilearn.config.json'), 'utf-8'))
  ...
  it('the wizard edit menu can re-run the database step', async () => {
    ...
    await setupCommand({})
    expect(fs.existsSync(path.join(home, '.omnilearn', 'edit-db', 'docker-compose.yml'))).toBe(true)
  })
  ...
  it('the wizard edit menu can re-run the admin step', async () => {
    ...
    await setupCommand({})
    const env = fs.readFileSync(path.join(home, '.omnilearn', 'edit-adm', '.env'), 'utf-8')
    expect(env).toContain('OMNILEARN_INITIAL_ADMIN_EMAIL=admin2@school.dev')
  })

  it('the wizard edit menu can re-run the optional-features step', async () => {
    wizardThenEdit('edit-feat', 5, { multiselect: [[]] }) // case 5
    await setupCommand({})
    expect(fs.existsSync(path.join(home, '.omnilearn', 'edit-feat', 'docker-compose.yml'))).toBe(true)
  })
  ...
  it('the wizard can go back through every step before proceeding', async () => {
    ...
    H.q.multiselect.push([])
    await setupCommand({})
    expect(fs.existsSync(path.join(home, '.omnilearn', 'bn', 'docker-compose.yml'))).toBe(true)
  })
  ...
  it('setupCommand dispatches to and completes the interactive EE wizard', async () => {
    ...
    const base = path.join(home, '.omnilearn')
  ...
  it('setup --ci --edition enterprise completes and returns', async () => {
    ...
    expect(fs.existsSync(path.join(home, '.omnilearn', 'ci-ee-ok', 'docker-compose.yml'))).toBe(true) // 173-174
  })
  ...
  it('the wizard writes a Caddyfile for an auto-SSL domain', async () => {
    ...
    H.q.multiselect.push([])
    await setupCommand({})
    expect(fs.existsSync(path.join(home, '.omnilearn', 'autossl', 'extra', 'Caddyfile'))).toBe(true) // 565
  })

  it('the wizard warns and can overwrite an existing installation', async () => {
    const existing = path.join(home, '.omnilearn', 'dup')
    fs.mkdirSync(existing, { recursive: true })
    fs.writeFileSync(path.join(existing, 'omnilearn.config.json'), '{}') // pre-existing install
    H.q.select.push('community', 'stable', 'continue', 'local', 'ai', 'local', 'continue', 'continue', 'continue', 'confirm')
  ...
  it('the wizard exits when an existing-install overwrite is declined', async () => {
    const existing = path.join(home, '.omnilearn', 'dup2')
    fs.mkdirSync(existing, { recursive: true })
    fs.writeFileSync(path.join(existing, 'omnilearn.config.json'), '{}')
    H.q.select.push('community', 'stable')
    H.q.text.push('dup2')
  ...
  it('the wizard start path warns on a health timeout but still completes', async () => {
    ...
    H.q.multiselect.push([])
    await setupCommand({})
    expect(fs.existsSync(path.join(home, '.omnilearn', 'timeout-wiz', 'docker-compose.yml'))).toBe(true)
  })
  ...
  it('the interactive wizard can start services after generating (startNow=yes)', async () => {
    ...
    await setupCommand({})
    expect(fs.existsSync(path.join(home, '.omnilearn', 'started', 'docker-compose.yml'))).toBe(true)
  })
  ...
  it('setup --ci with start warns when the health check times out', async () => {
    ...
    expect(fs.existsSync(path.join(home, '.omnilearn', 'slow', 'docker-compose.yml'))).toBe(true)
  })
  ...
  it('update on an enterprise install runs the EE upgrade path', async () => {
    const dir = path.join(home, '.omnilearn', 'ee')
  ...
  function seedInstall(dir: string) {
    ...
    fs.writeFileSync(path.join(dir, '.env'), 'OMNILEARN_DOMAIN=localhost\n')
    fs.writeFileSync(path.join(dir, 'docker-compose.yml'),
      'name: omnilearn-dep1\nservices:\n  omnilearn-app:\n    image: ghcr.io/omnilearn/app:1.4.0\n    container_name: omnilearn-app-dep1\n')
  }

  it('update aborts when the default pre-upgrade backup fails', async () => {
    seedInstall(path.join(home, '.omnilearn', 'test'))
  ...
  it('update handles an already-mounted content volume', async () => {
    const dir = path.join(home, '.omnilearn', 'mounted')
    fs.mkdirSync(dir, { recursive: true })
    fs.writeFileSync(path.join(dir, 'omnilearn.config.json'), JSON.stringify({
      version: '1.4.0', deploymentId: 'dep1', createdAt: '2026-01-01T00:00:00Z',
      installDir: dir, domain: 'localhost', httpPort: 8080,
  ...
  it('update skips content migration on an S3 install', async () => {
    const dir = path.join(home, '.omnilearn', 's3')
    fs.mkdirSync(dir, { recursive: true })
    fs.writeFileSync(path.join(dir, 'omnilearn.config.json'), JSON.stringify({
      version: '1.4.0', deploymentId: 'dep1', createdAt: '2026-01-01T00:00:00Z',
      installDir: dir, domain: 'localhost', httpPort: 8080,
  ...
  it('EE update exits when the migration fails', async () => {
    const dir = path.join(home, '.omnilearn', 'ee-migfail')
    fs.mkdirSync(dir, { recursive: true })
    fs.writeFileSync(path.join(dir, 'omnilearn.config.json'), JSON.stringify({
      version: '1.4.0', deploymentId: 'dep1', createdAt: '2026-01-01T00:00:00Z',
      installDir: dir, domain: 'learn.school.dev', httpPort: 443,
    ...
    fs.writeFileSync(path.join(dir, '.env'), 'DOMAIN=learn.school.dev\nEE_IMAGE_TAG=prod\n')
    fs.writeFileSync(path.join(dir, 'docker-compose.yml'), 'name: omnilearn-dep1\nservices:\n  api:\n    image: x\n')
    const m = (await import('node:child_process')).execSync as unknown as ReturnType<typeof vi.fn>
    m.mockImplementation(((cmd: string) => {
  ...
  it('EE update aborts (and reverts the tag) when the image pull fails', async () => {
    const dir = path.join(home, '.omnilearn', 'ee-pullfail')
    fs.mkdirSync(dir, { recursive: true })
    fs.writeFileSync(path.join(dir, 'omnilearn.config.json'), JSON.stringify({
      version: '1.4.0', deploymentId: 'dep1', createdAt: '2026-01-01T00:00:00Z',
      installDir: dir, domain: 'learn.school.dev', httpPort: 443,
  ...
  it('setup --ci rejects a short password before writing anything', async () => {
    ...
    expect(fs.existsSync(path.join(home, '.omnilearn', 'bad'))).toBe(false)
  })
  ...
  it('setup --ci exits when no admin password is given', async () => {
    ...
    expect(fs.existsSync(path.join(home, '.omnilearn', 'np'))).toBe(false)
  })
  ...
  it('update runs the full upgrade flow against a fixture install', async () => {
    ...
    const dir = path.join(home, '.omnilearn', 'test')
    fs.mkdirSync(dir, { recursive: true })
    fs.writeFileSync(path.join(dir, 'omnilearn.config.json'), JSON.stringify({
      version: '1.4.0', deploymentId: 'dep1', createdAt: '2026-01-01T00:00:00Z',
      installDir: dir, domain: 'localhost', httpPort: 8080,
    ...
    expect(fs.readFileSync(path.join(dir, 'docker-compose.yml'), 'utf-8'))
      .toContain('ghcr.io/omnilearn/app:latest')
  })
  ...
  it('setup --ci EE with external DB and Cloudflare DNS generates config', async () => {
    ...
    expect(fs.existsSync(path.join(home, '.omnilearn', 'ee-ext', 'docker-compose.yml'))).toBe(true)
  })
  ...
  it('setup --ci EE agency tenancy generates wildcard-domain files', async () => {
    ...
    const dir = path.join(home, '.omnilearn', 'ee-agency')
    const cfg = JSON.parse(fs.readFileSync(path.join(dir, 'omnilearn.config.json'), 'utf-8'))
  ...
  it('EE interactive overwrites an existing install when confirmed', async () => {
    const dir = path.join(home, '.omnilearn', 'ee-int')
    fs.mkdirSync(dir, { recursive: true })
  ...
  it('EE interactive cancels when the overwrite is declined', async () => {
    const dir = path.join(home, '.omnilearn', 'ee-int2')
    fs.mkdirSync(dir, { recursive: true })
    fs.writeFileSync(path.join(dir, '.env'), 'X=1\n')
    H.q.select.push('single')
    H.q.password.push('lh_live_TESTKEY', 'password123')
  ...
  it('EE interactive starts the stack when startNow is confirmed', async () => {
    ...
    expect(fs.existsSync(path.join(home, '.omnilearn', 'ee-int-start', 'docker-compose.yml'))).toBe(true)
  })
  ...
  it('EE setup with start reports OSS mode when the license is not active', async () => {
    ...
    expect(fs.existsSync(path.join(home, '.omnilearn', 'ee-oss', 'docker-compose.yml'))).toBe(true)
  })
  ...
  it('EE setup with start warns when EE readiness times out', async () => {
    ...
    expect(fs.existsSync(path.join(home, '.omnilearn', 'ee-to', 'docker-compose.yml'))).toBe(true)
  })
  ...
  it('setup --ci --edition enterprise WITH start logs in, pulls and boots EE', async () => {
    ...
    expect(fs.existsSync(path.join(home, '.omnilearn', 'ee-start', 'docker-compose.yml'))).toBe(true)
  })
  ...
  it('update --to <version> resolves the tag via GHCR and pins it', async () => {
    ...
    expect(fs.readFileSync(path.join(dir, 'docker-compose.yml'), 'utf-8'))
      .toContain('ghcr.io/omnilearn/app:1.2.6')
  })
})
...
describe('docker log streamers', () => {
  ...
  it('dockerLogsMulti tails each named container', () => {
    dockerLogsMulti(['omnilearn-app-dep1', 'omnilearn-db-dep1'])
    expect(spawnMock).toHaveBeenCalledTimes(2)
    expect(spawnMock.mock.calls[0][1]).toContain('omnilearn-app-dep1')
    expect(spawnMock.mock.calls[1][1]).toContain('omnilearn-db-dep1')
  })
  ...
  it('dockerExecInteractive runs  and records the exit code', async () => {
    ...
    dockerExecInteractive('omnilearn-app-dep1', '/bin/sh')
    const call = ss.mock.calls.at(-1) as [string, string[]]
    expect(call[0]).toBe('docker')
    expect(call[1].slice(0, 4)).toEqual(['exec', '-it', 'omnilearn-app-dep1', '/bin/sh'])
