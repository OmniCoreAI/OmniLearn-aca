...
# ─────────────────────────────────────────────────────────────
# OmniLearn — Install Script (macOS & Linux)
...
echo ""
echo -e "${BOLD}OmniLearn Installer${RESET}"
...
if command -v docker >/dev/null 2>&1; then
  ...
  if [ "$PLATFORM" = "macos" ]; then
    ...
    if ! docker info >/dev/null 2>&1; then
      ...
      echo -e "${DIM}  npx omnilearn${RESET}"
      exit 0
    fi
...
echo ""
echo -e "${CYAN}Launching OmniLearn setup...${RESET}"
echo ""

# Build the launch command — reattach stdin from /dev/tty when piped from curl
if [ ! -t 0 ]; then
  LAUNCH_CMD="npx omnilearn@latest setup </dev/tty"
else
  LAUNCH_CMD="npx omnilearn@latest setup"
fi
