...
export function migrateContentVolume(
  ...
  if (fs.existsSync(envPath)) {
    ...
    if (/^OMNILEARN_CONTENT_DELIVERY_TYPE\s*=\s*s3api\s*$/m.test(env)) {
      return { status: 'skipped_s3' }
    }
  }

  const projectName =
    compose.match(/^name:\s*(\S+)/m)?.[1] ?? `omnilearn-${deploymentId}`
  const volumeFull = `${projectName}_omnilearn_content_${deploymentId}`
  const containerName = `omnilearn-app-${deploymentId}`
...
export function patchComposeAddContentVolume(
  ...
  const mountLine = `      - omnilearn_content_${deploymentId}:${CONTENT_PATH}`

  const appStart = compose.indexOf('omnilearn-app:')
  if (appStart === -1) {
    throw new Error('omnilearn-app service not found in docker-compose.yml')
  }
  const networksIdx = compose.indexOf('\n    networks:', appStart)
  if (networksIdx === -1) {
    throw new Error('No service-level networks: block under omnilearn-app')
  }
  ...
  const volumeEntry = `  omnilearn_content_${deploymentId}:`
