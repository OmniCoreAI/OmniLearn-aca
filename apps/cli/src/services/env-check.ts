...
const WEB_ENV: AppEnvSpec = {
  ...
  vars: [
    {
      name: 'NEXT_PUBLIC_OMNILEARN_BACKEND_URL',
      required: true,
...
const COLLAB_ENV: AppEnvSpec = {
  ...
  vars: [
    ...
    },
    {
      ...
      required: true,
      description: 'OmniLearn API base URL',
      defaultValue: 'http://localhost:1338',
    },
    {
      name: 'OMNILEARN_AUTH_JWT_SECRET_KEY',
...
export async function checkDevEnv(root: string): Promise<boolean> {
  ...
  const jwtSecret = apiExisting.get('OMNILEARN_AUTH_JWT_SECRET_KEY') || generateJwtSecret()
  ...
  for (const app of ALL_APPS) {
    ...
    for (const v of app.vars) {
      ...
      if (v.name === 'OMNILEARN_AUTH_JWT_SECRET_KEY') {
