...
import type { OmniLearnConfigJson, SetupConfig } from '../types.js'

export function writeConfig(config: SetupConfig): void {
  const data: OmniLearnConfigJson = {
    version: VERSION,
...
export function readConfig(dir?: string): OmniLearnConfigJson | null {
...
export function listInstallations(): { name: string; dir: string; config: OmniLearnConfigJson }[] {
  const baseDir = path.join(os.homedir(), '.omnilearn')
  if (!fs.existsSync(baseDir)) return []

  const results: { name: string; dir: string; config: OmniLearnConfigJson }[] = []
  try {
...
export function findInstallDir(): string {
  ...
  if (installations.length > 1) {
    const running = installations.find((i) =>
      isContainerRunning(`omnilearn-app-${i.config.deploymentId}`)
    )
