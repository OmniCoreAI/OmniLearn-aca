...
const NPM_REGISTRY_URL = 'https://registry.npmjs.org/omnilearn'
...
export async function checkForUpdates(): Promise<void> {
  try {
    ...
    if (compareVersions(latest, VERSION) > 0) {
      ...
      console.log(pc.dim(`  Run: npx omnilearn@latest`))
      console.log()
    }
  } catch {
...
/**
 * Resolve the Docker image tag for the OmniLearn app.
 *
 * - channel 'dev'    → always returns ghcr.io/omnilearn/app:dev
...
export async function resolveAppImage(
  ...
  try {
    ...
    const releasesResp = await fetch(
      'https://api.github.com/repos/omnilearn/omnilearn/releases',
      {
    ...
    const manifestResp = await fetch(
      `https://ghcr.io/v2/omnilearn/app/manifests/${appVersion}`,
      {
        signal: AbortSignal.timeout(5000),
        headers: {
