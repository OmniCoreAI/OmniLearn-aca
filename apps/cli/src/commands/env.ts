...
const CATEGORIES: Record<string, { label: string; keys: string[] }> = {
  domain: {
    ...
    keys: [
      'OMNILEARN_DOMAIN', 'HTTP_PORT', 'NEXT_PUBLIC_OMNILEARN_API_URL',
      'NEXT_PUBLIC_OMNILEARN_BACKEND_URL', 'NEXT_PUBLIC_OMNILEARN_DOMAIN',
      'NEXT_PUBLIC_OMNILEARN_TOP_DOMAIN', 'NEXT_PUBLIC_OMNILEARN_MULTI_ORG',
      'NEXT_PUBLIC_OMNILEARN_DEFAULT_ORG', 'NEXT_PUBLIC_OMNILEARN_HTTPS',
      'NEXTAUTH_URL',
    ],
  },
  database: {
    ...
    keys: [
      'OMNILEARN_SQL_CONNECTION_STRING', 'OMNILEARN_REDIS_CONNECTION_STRING',
  ...
  security: {
    label: 'Security',
    keys: [
      'NEXTAUTH_SECRET', 'OMNILEARN_AUTH_JWT_SECRET_KEY',
      'OMNILEARN_INITIAL_ADMIN_EMAIL', 'OMNILEARN_INITIAL_ADMIN_PASSWORD',
      'OMNILEARN_COOKIE_DOMAIN',
    ],
  },
  ai: {
    label: 'AI',
    keys: ['OMNILEARN_GEMINI_API_KEY', 'OMNILEARN_IS_AI_ENABLED'],
  },
  email: {
    label: 'Email',
    keys: ['OMNILEARN_RESEND_API_KEY', 'OMNILEARN_SYSTEM_EMAIL_ADDRESS'],
  },
  s3: {
    label: 'S3 Storage',
    keys: [
      'OMNILEARN_CONTENT_DELIVERY_TYPE', 'OMNILEARN_S3_API_BUCKET_NAME',
      'OMNILEARN_S3_API_ENDPOINT_URL',
    ],
  },
  oauth: {
    label: 'OAuth',
    keys: [
      'OMNILEARN_GOOGLE_CLIENT_ID', 'OMNILEARN_GOOGLE_CLIENT_SECRET',
...
export async function envCommand() {
  ...
  if (!config) {
    p.log.error('No OmniLearn installation found. Run setup first.')
    process.exit(1)
  }
  ...
  p.intro(pc.cyan('OmniLearn Environment Editor'))
