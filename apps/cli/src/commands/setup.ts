...
async function stepInstallDir(): Promise<string | typeof BACK> {
  const baseDir = path.join(os.homedir(), '.omnilearn')

  p.log.info(`All installations are stored in ${pc.cyan('~/.omnilearn/<name>')}`)
  ...
  if (fs.existsSync(path.join(resolved, 'omnilearn.config.json'))) {
    p.log.warn(`~/.omnilearn/${name} already contains a OmniLearn installation.`)
...
export async function setupCommand(options: SetupOptions) {
  ...
  if (options.ci) {
    ...
    console.log(`Setting up OmniLearn in ~/.omnilearn/${installName}`)
    ...
    if (options.start !== false) {
      ...
      try {
        ...
        if (healthy) {
          ...
          if (seeded) {
            console.log('OmniLearn is ready!')
          } else {
            console.error('')
            console.error(`API is healthy but organization "${config.orgSlug}" was not seeded.`)
            console.error('Run `npx omnilearn logs` to inspect; pin a different image tag and re-run.')
            console.error('')
            process.exit(1)
          }
        } else {
  ...
  // ─── Interactive mode ───────────────────────────────────────
  await printBanner()
  p.intro(pc.cyan('OmniLearn Setup Wizard'))
  ...
  const s0 = p.spinner()
  s0.start('Resolving OmniLearn image version')
  ...
  if (startNow) {
    ...
    try {
      ...
      if (portAllocated) {
        ...
        p.log.info(
          'Fix one of these and re-run `omnilearn start` (your config is already saved):\n' +
          `  • Stop whatever is using port ${config.httpPort}, or\n` +
          `  • Re-run \`omnilearn setup\` and pick a different port, or\n` +
    ...
    // Health check
    const s3 = p.spinner()
    s3.start('Waiting for OmniLearn to be ready (up to 3 minutes)')
    ...
    if (healthy) {
      const seeded = await waitForOrgSeed(`http://localhost:${config.httpPort}`, config.orgSlug)
      if (seeded) {
        s3.stop('OmniLearn is ready!')
      } else {
        s3.stop(`API is healthy but organization "${config.orgSlug}" was not seeded`)
        p.log.error(
          `\`/api/v1/orgs/slug/${config.orgSlug}\` returns no org. Run \`omnilearn logs\` to inspect, ` +
      ...
      p.log.warn('OmniLearn may still be starting. Check status with:')
      p.log.message(`  cd ${finalDir} && docker compose ps`)
    }

    // Success
    p.log.success(pc.green(pc.bold('OmniLearn is installed!')))
    p.log.message([
      '',
      ...
      `  ${pc.dim('$')} npx omnilearn start    ${pc.dim('Start services')}`,
      `  ${pc.dim('$')} npx omnilearn stop     ${pc.dim('Stop services')}`,
      `  ${pc.dim('$')} npx omnilearn logs     ${pc.dim('View logs')}`,
      `  ${pc.dim('$')} npx omnilearn config   ${pc.dim('Show configuration')}`,
      `  ${pc.dim('$')} npx omnilearn backup   ${pc.dim('Backup & restore')}`,
      `  ${pc.dim('$')} npx omnilearn deployments ${pc.dim('Manage deployments')}`,
      `  ${pc.dim('$')} npx omnilearn doctor   ${pc.dim('Diagnose issues')}`,
      `  ${pc.dim('$')} npx omnilearn shell    ${pc.dim('Container shell')}`,
      '',
    ].join('\n'))
  } else {
