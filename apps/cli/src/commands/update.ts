...
const GHCR_BASE = 'ghcr.io/omnilearn/app'

async function resolveTag(version: string): Promise<boolean> {
  try {
    const tokenResp = await fetch(
      'https://ghcr.io/token?scope=repository:omnilearn/app:pull',
    ...
    const manifestResp = await fetch(
      `https://ghcr.io/v2/omnilearn/app/manifests/${version}`,
      {
        signal: AbortSignal.timeout(5000),
        headers: {
          Accept:
...
export async function updateCommand(options: { version?: string; migrate?: boolean; backup?: boolean }) {
  ...
  if (config.edition === 'enterprise') {
    p.intro(pc.cyan('Upgrading OmniLearn Enterprise'))
  ...
  if (targetVersion) {
    p.intro(pc.cyan(`Updating OmniLearn to v${targetVersion}`))
  } else {
    p.intro(pc.cyan('Updating OmniLearn to latest'))
  }
  ...
  try {
    ...
    if (targetVersion) {
      ...
      if (!exists) {
        ...
        if (!existsWithV) {
          ...
          p.log.error(
            `Version ${targetVersion} not found on ghcr.io/omnilearn/app`,
          )
          process.exit(1)
        }
    ...
    s.start('Waiting for OmniLearn to be ready')
    await waitForHealth(`http://localhost:${config.httpPort}`)
    s.stop('OmniLearn is up')
    ...
    if (options.migrate !== false) {
      ...
      p.log.info('  docker compose exec omnilearn-app sh -c "cd /app/api && uv run alembic upgrade head"')
    }

    if (targetVersion) {
      p.log.success(`OmniLearn has been updated to v${targetVersion}!`)
    } else {
      p.log.success('OmniLearn has been updated to the latest version!')
    }
  } catch {
