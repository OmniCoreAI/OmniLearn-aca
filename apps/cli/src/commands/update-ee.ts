...
import type { OmniLearnConfigJson } from '../types.js'
...
export interface EditionLayout {
  appService: string      // compose service running the API (EE: 'api', CE: 'omnilearn-app')
...
export function isExternalDbInstall(installDir: string): boolean {
  return !!readEnvVar(installDir, 'OMNILEARN_SQL_CONNECTION_STRING')
}
...
export function backupDatabase(config: OmniLearnConfigJson, layout: EditionLayout, ui: UpdateLog): string {
  ...
  if (isExternalDbInstall(dir)) {
    const conn = readEnvVar(dir, 'OMNILEARN_SQL_CONNECTION_STRING') || ''
    ...
    execSync(
      `docker compose exec -T ${layout.dbService} sh -c 'pg_dump -U "$\{POSTGRES_USER:-omnilearn}" "$\{POSTGRES_DB:-omnilearn}"' | gzip > ${JSON.stringify(out)}`,
...
export async function updateEnterprise(config: OmniLearnConfigJson, options: EeUpdateOptions): Promise<void> {
  const dir = config.installDir
  const ui: UpdateLog = {
  ...
  ui.log(`Upgrading OmniLearn Enterprise: ${pc.dim(currentTag)} → ${pc.cyan(targetTag)}`)
  ...
  const license = readEnvVar(dir, 'OMNILEARN_LICENSE_KEY')
  if (!license) die('OMNILEARN_LICENSE_KEY not found in .env — cannot pull EE images.')
  ...
  if (ready === 'oss') ui.warn('API came up in OSS mode — license not active. Check `omnilearn logs`.')
  ...
  ui.ok(`OmniLearn Enterprise upgraded to ${targetTag}.`)
