...
const SERVICES = ['omnilearn-app', 'db', 'redis'] as const
...
function showDeployments() {
  ...
  try {
    psOutput = execSync(
      'docker ps -a --filter "name=omnilearn-app-" --format "{{.Names}}\\t{{.Status}}\\t{{.Image}}"',
      { stdio: 'pipe' },
    ).toString().trim()
  } catch {
  ...
  if (!psOutput) {
    p.log.info('No OmniLearn deployments found.')
    p.log.message(pc.dim('  Run npx omnilearn setup to create one.'))
    return
  }
  ...
  try {
    allOutput = execSync(
      'docker ps -a --filter "name=omnilearn-" --format "{{.Names}}\\t{{.Status}}\\t{{.Image}}"',
      { stdio: 'pipe' },
    ).toString().trim()
  } catch {
    allOutput = psOutput
  ...
  for (const line of allOutput.split('\n')) {
    ...
    const match = name.match(/omnilearn-\w+-([a-f0-9]+)$/)
    if (!match) continue
...
export async function deploymentsCommand() {
  p.intro(pc.cyan('OmniLearn Deployments'))
