...
const REQUIRED_ENV_VARS = [
  ...
  'OMNILEARN_REDIS_CONNECTION_STRING',
...
const EE_REQUIRED_ENV_VARS_BASE = [
  'OMNILEARN_LICENSE_KEY',
  'ACME_EMAIL',
  'EE_IMAGE_TAG',
  'DB_PASSWORD',
  'OMNILEARN_AUTH_JWT_SECRET_KEY',
  'COLLAB_INTERNAL_KEY',
  'OMNILEARN_INITIAL_ADMIN_EMAIL',
  'OMNILEARN_INITIAL_ADMIN_PASSWORD',
]
...
export async function doctorCommand() {
  ...
  p.intro(pc.cyan('OmniLearn Doctor'))
  ...
  if (!config) {
    p.log.warn('No OmniLearn installation found. Skipping deployment checks.')
    p.outro(pc.dim('Done'))
    return
  }
  ...
  if (containers.length === 0) {
    warn('No containers found', 'Run: npx omnilearn start')
  } else {
    for (const c of containers) {
      ...
      if (isUp) {
        ...
        fail(`${svcName} is restarting`, 'Check logs: npx omnilearn logs')
      } else {
        fail(`${svcName} — ${c.status}`, 'Run: npx omnilearn start')
      }
    }
  }
  ...
  if (portFree) {
    ...
    if (hasRunning) {
      pass(`Port ${config.httpPort} in use (by OmniLearn services)`)
    } else {
  ...
  if (!fs.existsSync(envPath)) {
    fail('.env file missing', 'Run setup again: npx omnilearn setup')
  } else {
