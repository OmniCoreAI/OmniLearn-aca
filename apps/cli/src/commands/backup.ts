...
async function createBackup() {
  ...
  const backupName = `omnilearn-backup-${timestamp}`
  ...
  p.log.message([
    ...
    '',
    `  ${pc.dim('Restore with:')} npx omnilearn backup --restore ${archivePath}`,
    '',
  ].join('\n'))
}
...
async function restoreBackup(archivePath: string) {
  ...
  p.log.info('You may want to restart services: npx omnilearn stop && npx omnilearn start')
}
...
export async function backupCommand(archivePath?: string, options?: { restore?: boolean }) {
  ...
  if (action === 'create') {
    ...
    const filePath = await p.text({
      ...
      placeholder: './backups/omnilearn-backup-*.tar.gz',
    })
