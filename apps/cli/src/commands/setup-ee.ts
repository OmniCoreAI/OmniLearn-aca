...
function readExistingSecrets(installDir: string): Partial<EeSecrets> {
  ...
  return {
    ...
    jwtSecret: get('OMNILEARN_AUTH_JWT_SECRET_KEY'),
...
async function startEe(config: SetupConfig, interactive: boolean, firstDeploy: boolean): Promise<void> {
  ...
  try {
    ...
    const m = `Failed to pull or start the EE stack. Inspect with \`omnilearn logs\` (cd ${dir} && docker compose logs).`
  ...
  if (ready === 'ee') {
    ...
    const m = 'API is up but in OSS mode — license not active. Check `omnilearn logs`.'
...
async function setupEnterpriseCi(options: EeSetupOptions): Promise<void> {
  ...
  const installDir = options.installDir
    ...
    : path.join(os.homedir(), '.omnilearn', installName)
  ...
  console.log(`Setting up OmniLearn Enterprise (${tenancy}) in ${installDir}`)
  try {
...
async function setupEnterpriseInteractive(options: EeSetupOptions): Promise<void> {
  ...
  const startNow = exit(await p.confirm({ message: 'Start OmniLearn Enterprise now?', initialValue: true })) as boolean
  if (!startNow) {
