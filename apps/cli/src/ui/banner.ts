...
function buildInfoBox(): string[] {
  ...
  return [
    top,
    boxLine(pc.bold(pc.white('OmniLearn')) + pc.dim(` // v${VERSION}`)),
    boxLine(sep),
    boxLine(pc.white('Deploy OmniLearn with a single command.')),
    boxLine(pc.white('Handles configuration, Docker, SSL, DB.')),
    empty,
    boxLine(pc.white('> ') + pc.dim('npx omnilearn@latest')),
    bot,
  ]
}
