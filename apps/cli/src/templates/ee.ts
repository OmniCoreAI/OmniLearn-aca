...
// partners.omnilearn.app/templates, ported to the CLI so the one tool can
// deploy both Community and Enterprise editions.
//
// The EE stack is six services — db (pgvector), redis, caddy (auto-TLS),
// api, web, collab — pulling enterprise images from images.omnilearn.app
...
export function generateEeDockerCompose(config: SetupConfig): string {
  ...
  const sqlConn = external
    ? '${OMNILEARN_SQL_CONNECTION_STRING:?OMNILEARN_SQL_CONNECTION_STRING is required}'
    : 'postgresql://${DB_USER:-omnilearn}:${DB_PASSWORD}@db:5432/${DB_NAME:-omnilearn}'
  ...
  const dbService = external
    ...
    environment:
      POSTGRES_USER: \${DB_USER:-omnilearn}
      POSTGRES_PASSWORD: \${DB_PASSWORD:?DB_PASSWORD is required}
      POSTGRES_DB: \${DB_NAME:-omnilearn}
    volumes:
      - db_data:/var/lib/postgresql/data
      - ./pgvector-init.sql:/docker-entrypoint-initdb.d/01-init.sql:ro
    restart: unless-stopped
    healthcheck:
      test: ["CMD", "pg_isready", "-U", "\${DB_USER:-omnilearn}"]
      interval: 5s
      timeout: 3s
      retries: 20
...
networks:
  ...
  const tenancyEnv = agency
    ? `      OMNILEARN_TENANCY: multi
      OMNILEARN_DOMAIN: \${AGENCY_DOMAIN}
      OMNILEARN_FRONTEND_DOMAIN: \${AGENCY_DOMAIN}
      OMNILEARN_COOKIE_DOMAIN: .\${AGENCY_DOMAIN}
      OMNILEARN_COOKIE_DOMAIN_ALLOW_BROAD: \${OMNILEARN_COOKIE_DOMAIN_ALLOW_BROAD:-false}`
    : `      OMNILEARN_TENANCY: single
      OMNILEARN_DOMAIN: \${DOMAIN}
      OMNILEARN_FRONTEND_DOMAIN: \${DOMAIN}`

  const webEnv = agency
    ? `      NEXT_PUBLIC_OMNILEARN_BACKEND_URL: https://\${AGENCY_DOMAIN}/
      NEXT_PUBLIC_OMNILEARN_API_URL: https://\${AGENCY_DOMAIN}/api/v1/
      NEXT_PUBLIC_OMNILEARN_HTTPS: "true"
      NEXT_PUBLIC_OMNILEARN_DOMAIN: \${AGENCY_DOMAIN}
      NEXT_PUBLIC_OMNILEARN_TOP_DOMAIN: \${AGENCY_DOMAIN}
      NEXT_PUBLIC_OMNILEARN_MULTI_ORG: "true"`
    : `      NEXT_PUBLIC_OMNILEARN_BACKEND_URL: https://\${DOMAIN}/
      NEXT_PUBLIC_OMNILEARN_API_URL: https://\${DOMAIN}/api/v1/
      NEXT_PUBLIC_OMNILEARN_HTTPS: "true"
      NEXT_PUBLIC_OMNILEARN_DOMAIN: \${DOMAIN}
      NEXT_PUBLIC_OMNILEARN_TOP_DOMAIN: \${DOMAIN}
      NEXT_PUBLIC_OMNILEARN_MULTI_ORG: "false"
      NEXT_PUBLIC_OMNILEARN_DEFAULT_ORG: default`

  const adminEmailEnv = agency
    ? `      OMNILEARN_INITIAL_ADMIN_EMAIL: \${OMNILEARN_INITIAL_ADMIN_EMAIL:-admin@\${AGENCY_DOMAIN}}`
    : `      OMNILEARN_INITIAL_ADMIN_EMAIL: \${OMNILEARN_INITIAL_ADMIN_EMAIL:?OMNILEARN_INITIAL_ADMIN_EMAIL is required}`

  return `name: omnilearn-${id}

# OmniLearn Enterprise Edition (${agency ? 'agency / multi-tenant' : 'single-tenant'}).
# Generated by the OmniLearn CLI. Replace values in .env before booting.
...
${apiDepends}
    ...
    environment:
      ...
      OMNILEARN_LICENSE_KEY: \${OMNILEARN_LICENSE_KEY:?OMNILEARN_LICENSE_KEY is required}
      OMNILEARN_LICENSE_SERVER_URL: \${OMNILEARN_LICENSE_SERVER_URL:-${EE_LICENSE_SERVER}}
      OMNILEARN_DATA_DIR: /app/data

      # --- Tenancy ---
${tenancyEnv}

      # --- Auth + secrets ---
      OMNILEARN_AUTH_JWT_SECRET_KEY: \${OMNILEARN_AUTH_JWT_SECRET_KEY:?OMNILEARN_AUTH_JWT_SECRET_KEY is required (min 32 chars)}
      COLLAB_INTERNAL_KEY: \${COLLAB_INTERNAL_KEY:?COLLAB_INTERNAL_KEY is required}
      OMNILEARN_INITIAL_ADMIN_PASSWORD: \${OMNILEARN_INITIAL_ADMIN_PASSWORD:?OMNILEARN_INITIAL_ADMIN_PASSWORD is required}
${adminEmailEnv}

      # --- Storage ---
      OMNILEARN_SQL_CONNECTION_STRING: ${sqlConn}
...
${collabDepends}    environment:
      ...
      OMNILEARN_AUTH_JWT_SECRET_KEY: \${OMNILEARN_AUTH_JWT_SECRET_KEY}
      OMNILEARN_SQL_CONNECTION_STRING: ${sqlConn}
...
# Generated by the OmniLearn CLI.
...
export function generateEeCaddyfile(config: SetupConfig): string {
  ...
  if (!agency) {
    return `# Caddyfile — single-tenant OmniLearn EE. Generated by the OmniLearn CLI.
...
`
  ...
  return `# Caddyfile — agency OmniLearn EE with on-demand TLS for custom domains.
# Generated by the OmniLearn CLI.
...
export function generateEeEnv(config: SetupConfig, secrets: EeSecrets): string {
  ...
  const external = isExternalDb(config)
  const lines = [
    '# OmniLearn Enterprise Edition — generated by the OmniLearn CLI.',
    '# Contains secrets — do not commit.',
    '',
    `OMNILEARN_LICENSE_KEY=${quoteEnvValue(config.licenseKey || '')}`,
  ...
  if (external) {
    lines.push('', '# External database (e.g. Supabase) — the in-container db is not used.',
      `OMNILEARN_SQL_CONNECTION_STRING=${quoteEnvValue(config.externalDbUrl || '')}`)
  } else {
    lines.push('', 'DB_USER=omnilearn', `DB_PASSWORD=${secrets.dbPassword}`, 'DB_NAME=omnilearn')
  }
  lines.push(
    '',
    `OMNILEARN_AUTH_JWT_SECRET_KEY=${secrets.jwtSecret}`,
    `COLLAB_INTERNAL_KEY=${secrets.collabKey}`,
    '',
    `OMNILEARN_INITIAL_ADMIN_EMAIL=${quoteEnvValue(config.adminEmail)}`,
    `OMNILEARN_INITIAL_ADMIN_PASSWORD=${quoteEnvValue(config.adminPassword)}`,
  )
  if (isCloudflareDns(config)) {
    lines.push('', `CLOUDFLARE_API_TOKEN=${quoteEnvValue(config.cfApiToken || '')}`)
  }
  if (agency) {
    lines.push('', 'OMNILEARN_COOKIE_DOMAIN_ALLOW_BROAD=false')
  }
  return lines.join('\n') + '\n'
}
...
export function generateEeLocalTlsOverride(): string {
  return `# Written by the OmniLearn CLI --local-tls.
