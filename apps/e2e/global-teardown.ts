 ...
 * the compose file the CLI generated under ~/.omnilearn/<name>/. We avoid
 * `omnilearn stop` here because it can prompt interactively when more than one
...
export default async function globalTeardown(): Promise<void> {
  ...
  const composePath = join(homedir(), '.omnilearn', INSTALL_NAME, 'docker-compose.yml')
