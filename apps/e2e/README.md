# OmniLearn E2E — UI acceptance tests

Browser-driven, human-like end-to-end tests for OmniLearn. The suite boots a
real **self-host via the OmniLearn CLI** (`setup --ci`, pulling the published
...
| `E2E_CLI` | `npx --yes omnilearn@latest` | CLI used to boot (override for a local build). |
...
> instance (it passes against `omnilearn dev` / a branch-built image).
