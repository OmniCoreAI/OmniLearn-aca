# Tinybird Analytics for OmniLearn

OmniLearn uses [Tinybird](https://www.tinybird.co/) (managed ClickHouse) for analytics event ingestion and querying. This directory contains the datasource schema and reference pipe definitions.
...
```
OMNILEARN_TINYBIRD_API_URL=https://api.europe-west2.gcp.tinybird.co
OMNILEARN_TINYBIRD_INGEST_TOKEN=p.eyJ...
OMNILEARN_TINYBIRD_READ_TOKEN=p.eyJ...
```

- `OMNILEARN_TINYBIRD_API_URL` — Your workspace's regional API URL
- `OMNILEARN_TINYBIRD_INGEST_TOKEN` — Token with write permissions (Events API)
- `OMNILEARN_TINYBIRD_READ_TOKEN` — Token with read permissions (Query API / SQL)
...
curl -X POST \
  ...
  -H "Authorization: Bearer $OMNILEARN_TINYBIRD_INGEST_TOKEN" \
...
curl -X POST \
  ...
  -H "Authorization: Bearer $OMNILEARN_TINYBIRD_READ_TOKEN" \
...
OmniLearn uses the Tinybird **Query API** (`POST /v0/sql`) to run ClickHouse SQL directly, rather than deploying pipe endpoints. This approach:
...
The `OMNILEARN_TINYBIRD_INGEST_TOKEN` and/or `OMNILEARN_TINYBIRD_READ_TOKEN` env vars are not set. Set both to enable analytics.
