...
Three modes:
- 'saas': OMNILEARN_SAAS=true — plan-based gating, usage limits apply
...
- OMNILEARN_FORCE_EE=1 skips the license check when the EE folder is present.
...
def get_deployment_mode() -> DeploymentMode:
    ...
    """
    if get_omnilearn_config().general_config.saas_mode:
        return 'saas'
    ...
    if is_ee_available():
        ...
        # OMNILEARN_FORCE_EE=1 is only honoured in development_mode to
        # prevent accidental use in production deployments.
        if (
            os.environ.get('OMNILEARN_FORCE_EE') == '1'
            and get_omnilearn_config().general_config.development_mode
        ):
            return 'ee'
