...
# OmniLearn domain for CNAME instructions
OMNILEARN_DOMAIN = os.getenv('OMNILEARN_DOMAIN', 'omnilearn.io')
...
def is_own_agency_subdomain(domain: str, org_slug: str) -> bool:
    ...
    In agency (multi-tenant) deployments the whole `*.{OMNILEARN_DOMAIN}` space
    ...
    """
    base = (OMNILEARN_DOMAIN or "").lower().strip().split(":")[0]
    if not base or base in ("omnilearn.io", "localhost"):
        return False
...
def get_verification_instructions(domain: str, token: str, org_slug: str) -> CustomDomainVerificationInfo:
    ...
    if subdomain:
        txt_record_host = f"_omnilearn-verification.{subdomain}"
        cname_record_host = subdomain
    else:
        txt_record_host = "_omnilearn-verification"
        cname_record_host = "@"

    txt_record_value = f"omnilearn-verify={token}"
    cname_record_value = f"{org_slug}.{OMNILEARN_DOMAIN}"
...
async def verify_domain_dns(domain: CustomDomain, db_session: AsyncSession, org_slug: str) -> Tuple[bool, str]:
    ...
    if os.getenv('OMNILEARN_CUSTOM_DOMAIN_DEV_MODE') == 'true':
    ...
    if is_own_agency_subdomain(domain.domain, org_slug):
        logger.info(f"Auto-verifying {domain.domain} (own slug host under agency domain {OMNILEARN_DOMAIN})")
        domain.status = "verified"
        domain.verified_at = str(datetime.now())
        domain.last_check_at = str(datetime.now())
        domain.check_error = None
    ...
    try:
        ...
        # Check TXT record
        txt_host = f"_omnilearn-verification.{domain.domain}"
        expected_txt = f"omnilearn-verify={domain.verification_token}"
...
async def add_custom_domain(
    ...
    if auto_verified:
        logger.info(f"Auto-verified {domain} on add (subdomain of agency domain {OMNILEARN_DOMAIN})")
