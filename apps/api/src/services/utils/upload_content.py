...
async def upload_content(
    ...
    if content_delivery == "filesystem":
        ...
        s3 = boto3.client(
            "s3",
            endpoint_url=omnilearn_config.hosting_config.content_delivery.s3api.endpoint_url,
            config=botocore.config.Config(connect_timeout=10, read_timeout=60, retries={"max_attempts": 2}),
        )

        bucket_name = omnilearn_config.hosting_config.content_delivery.s3api.bucket_name or "omnilearn-media"
