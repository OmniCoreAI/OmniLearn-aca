...
def embedding_dimensions() -> int:
    cfg = get_omnilearn_config().ai_config
...
def build_embedding_model() -> EmbeddingModel:
    ...
    if prov in _GOOGLE_ALIASES:
        ...
        if not key:
            raise AINotConfiguredError(
                "Google embeddings require an API key (set OMNILEARN_GEMINI_API_KEY)."
            )
    ...
    if prov in _OPENAI_ALIASES:
        ...
        if not api_key:
            raise AINotConfiguredError(
                "OpenAI embeddings require an API key (set OMNILEARN_AI_API_KEY)."
            )
        return OpenAIEmbeddingModel(
    ...
    raise AINotConfiguredError(
        f"Provider '{prov}' has no embeddings API. Set OMNILEARN_AI_EMBEDDING_PROVIDER to "
        "'google', 'openai', or 'ollama' (with its credentials), or set OMNILEARN_GEMINI_API_KEY "
