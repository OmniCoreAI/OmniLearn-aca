...
def build_model(model_name: str) -> Model:
    ...
    # `~/.aws` profile) + AWS_REGION, so no OMNILEARN_AI_API_KEY is required. If one is set
    ...
    if not api_key:
        raise AINotConfiguredError(
            "AI provider API key not configured (set OMNILEARN_AI_API_KEY)"
        )
    ...
    if provider_id == "openrouter":
        ...
        return OpenAIChatModel(
            ...
            provider=OpenRouterProvider(
                api_key=api_key,
                app_title=getattr(lh_config, "site_name", None) or "OmniLearn",
            ),
        )
