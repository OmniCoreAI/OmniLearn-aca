...
async def _verify_google_token_audience(access_token: str) -> None:
    ...
    If ``OMNILEARN_GOOGLE_OAUTH_CLIENT_ID`` is not set we log a one-shot
    ...
    expected_client_id = os.environ.get("OMNILEARN_GOOGLE_OAUTH_CLIENT_ID")
    if not expected_client_id:
        if not _LOGGED_MISSING_GOOGLE_CLIENT_ID:
            logger.warning(
                "OMNILEARN_GOOGLE_OAUTH_CLIENT_ID is not set — Google OAuth "
...
async def signWithGoogle(
    ...
    # token impersonate any OmniLearn user whose address they knew. The body
    ...
    # take over the OmniLearn account. Accept only an explicit boolean ``True``
    ...
    # entirely, so a person who already had a OmniLearn account and accepted
