...
async def verify_platform_key(x_platform_key: str = Header(...)):
    expected_key = os.getenv("OMNILEARN_PLATFORM_API_KEY", "")
    if not expected_key:
        raise HTTPException(
            status_code=500,
            detail="OMNILEARN_PLATFORM_API_KEY is not configured on the server",
        )
...
@internal_router.post(
    ...
    responses={
        ...
        500: {"description": "OMNILEARN_PLATFORM_API_KEY is not configured on the server"},
    },
)
