...
async def api_export_courses_batch(
    ...
    **Required Permissions:**
    ...
    filename = f"omnilearn-export-batch-{timestamp}.zip"
...
@router.post(
    ...
    description=(
        "Upload and analyze a OmniLearn course export ZIP. Validates the package, "
...
async def api_analyze_import_package(
    ...
    """
    Analyze a OmniLearn course export package for import.
...
@router.get(
    ...
    description=(
        ...
        "and files) as a ZIP archive following the OmniLearn export format."
    ),
    responses={
...
async def api_export_course(
    ...
    The ZIP file follows the OmniLearn export format and can be imported
    into any OmniLearn instance.
    ...
    **Required Permissions:**
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"omnilearn-export-{course_uuid}-{timestamp}.zip"
