...
def _get_judge0_config():
    ...
    if not config.judge0_config:
        raise HTTPException(
            status_code=503,
            detail="Code execution is not configured. Set OMNILEARN_JUDGE0_API_URL.",
        )
...
def _read_storage_file(file_path: str) -> bytes:
    ...
    if content_delivery == "filesystem":
        ...
        bucket = config.hosting_config.content_delivery.s3api.bucket_name or "omnilearn-media"
        try:
