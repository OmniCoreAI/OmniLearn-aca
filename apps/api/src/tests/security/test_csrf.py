...
class TestCSRFOriginValidation:
    ...
    def test_allowed_origin_exact_match(self):
        with patch("src.security.csrf.get_omnilearn_config", return_value=_make_mock_config(
    ...
    def test_invalid_regex_is_logged_and_disabled(self):
        ...
        ) as mock_error, patch("src.security.csrf.get_omnilearn_config", return_value=_make_mock_config(
            allowed_origins=[],
            allowed_regexp=r"[",
        )):
            from src.security.csrf import CSRFProtectionMiddleware
