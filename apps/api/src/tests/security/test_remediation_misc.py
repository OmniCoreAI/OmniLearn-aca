...
def test_single_label_cookie_domain_refused_in_prod(monkeypatch):
    """
    F-17: ``OMNILEARN_COOKIE_DOMAIN=.com`` must raise at config load in
    non-dev mode. Single-label public parents are always a bug.
    """
    monkeypatch.setenv("OMNILEARN_AUTH_JWT_SECRET_KEY", "x" * 40)
    monkeypatch.setenv("OMNILEARN_DEVELOPMENT_MODE", "false")
    monkeypatch.setenv("OMNILEARN_COOKIE_DOMAIN", ".com")
    ...
    with pytest.raises(ValueError) as exc:
        cfg_module.get_omnilearn_config()
...
def test_dotted_cookie_domain_ok(monkeypatch):
    ...
    monkeypatch.setenv("OMNILEARN_AUTH_JWT_SECRET_KEY", "x" * 40)
    monkeypatch.setenv("OMNILEARN_DEVELOPMENT_MODE", "false")
    monkeypatch.setenv("OMNILEARN_COOKIE_DOMAIN", ".app.example.com")
    monkeypatch.setenv("OMNILEARN_COOKIE_DOMAIN_ALLOW_BROAD", "true")
    ...
    importlib.reload(cfg_module)
    cfg = cfg_module.get_omnilearn_config()
