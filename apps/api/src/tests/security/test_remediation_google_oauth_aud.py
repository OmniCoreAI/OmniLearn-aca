...
When ``OMNILEARN_GOOGLE_OAUTH_CLIENT_ID`` is set, tokens minted for a
...
async def test_rejects_token_minted_for_different_oauth_client(monkeypatch):
    ...
    """
    monkeypatch.setenv("OMNILEARN_GOOGLE_OAUTH_CLIENT_ID", "our-client-id")
...
async def test_warns_but_accepts_when_client_id_unset(monkeypatch, caplog):
    """
    No OMNILEARN_GOOGLE_OAUTH_CLIENT_ID set — log once, fall through.
    Existing deployments without the env var keep working.
    """
    monkeypatch.delenv("OMNILEARN_GOOGLE_OAUTH_CLIENT_ID", raising=False)
