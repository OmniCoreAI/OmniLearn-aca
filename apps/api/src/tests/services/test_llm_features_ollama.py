...
(OMNILEARN_AI_* env vars -> get_omnilearn_config() -> build_model), exactly as a deployment
...
def _ollama_env(monkeypatch):
    ...
    monkeypatch.setenv("OMNILEARN_AI_PROVIDER", "ollama")
    monkeypatch.setenv("OMNILEARN_AI_BASE_URL", f"http://{OLLAMA_HOST}:{OLLAMA_PORT}/v1")
    for tier in ("FAST", "STANDARD", "PRO"):
        monkeypatch.setenv(f"OMNILEARN_AI_MODEL_{tier}", OLLAMA_MODEL)
    # Embeddings follow the same (ollama) provider; use a local embedding model.
    monkeypatch.setenv("OMNILEARN_AI_EMBEDDING_MODEL", OLLAMA_EMBED_MODEL)
