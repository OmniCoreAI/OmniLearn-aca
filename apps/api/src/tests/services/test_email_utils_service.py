...
def _config(**overrides):
    hosting = SimpleNamespace(
        ...
        domain=overrides.pop("domain", "omnilearn.app"),
        frontend_domain=overrides.pop("frontend_domain", "app.omnilearn.app"),
...
class TestEmailUtilsService:
    def test_is_allowed_base_url_matches_all_supported_sources(self):
        with patch(
            "src.services.email.utils.get_omnilearn_config",
            ...
            {"OMNILEARN_PLATFORM_URL": "https://www.platform.test"},
            clear=False,
        ):
    ...
    def test_is_allowed_base_url_pins_to_configured_host_in_single_tenancy(self):
        with patch(
            ...
            assert _is_allowed_base_url("https://app.omnilearn.app")
            assert _is_allowed_base_url("https://omnilearn.app")
            assert _is_allowed_base_url("https://www.omnilearn.app")
    ...
    @pytest.mark.parametrize(
        "ssl,expected",
        [(True, "https://acme.omnilearn.app"), (False, "http://acme.omnilearn.app")],
    )
    ...
    def test_get_base_url_from_request_uses_frontend_then_request_url(self):
        ...
        url_request = _request({}, scheme="http", server=("api.omnilearn.test", 8080))
        ...
        with patch(
            ...
            "src.services.email.utils.get_omnilearn_config",
            return_value=_config(frontend_domain="frontend.omnilearn.app"),
        ):
            assert (
                get_base_url_from_request(frontend_request)
                == "https://frontend.omnilearn.app"
            )
        ...
        with patch(
            ...
            assert get_base_url_from_request(url_request) == "http://api.omnilearn.test:8080"
    ...
    def test_send_email_routes_to_resend_and_smtp(self):
        ...
        assert mock_resend_send.call_args.args[0] == {
            "from": "OmniLearn <system@test.com>",
            "to": ["to@test.com"],
            "subject": "Hello",
            "html": "<p>Body</p>",
        }
        ...
        mock_smtp.assert_called_once_with("smtp.omnilearn.test", 2525, timeout=15)
...
class TestGetPrimaryVerifiedCustomDomain:
    ...
    async def test_get_org_signup_base_url_uses_primary_custom_domain(self):
        ...
        with patch(
            "src.services.email.utils.get_omnilearn_config",
            return_value=_config(tenancy="multi", ssl=True, domain="omnilearn.app"),
        ), patch(
    ...
    async def test_get_org_signup_base_url_falls_through_when_no_custom_domain(self):
        ...
        assert url == "https://myorg.omnilearn.app"
