...
def _patch_ai_config(monkeypatch, **overrides):
    ...
    monkeypatch.setattr(
        provider_mod, "get_omnilearn_config", lambda: SimpleNamespace(ai_config=cfg)
    )
...
def test_build_model_bedrock_needs_no_api_key(monkeypatch):
    # Bedrock uses the standard AWS credential chain + region (not OMNILEARN_AI_API_KEY).
...
def _patch_embed_config(monkeypatch, **overrides):
    ...
    cfg = SimpleNamespace(**fields)
    monkeypatch.setattr(
        embeddings_mod, "get_omnilearn_config", lambda: SimpleNamespace(ai_config=cfg)
    )
