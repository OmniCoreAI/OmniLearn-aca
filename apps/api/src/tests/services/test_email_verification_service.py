...
class TestEmailVerificationService:
    ...
    def test_get_redis_connection_errors(self):
        with patch(
            "src.services.users.email_verification.get_omnilearn_config",
    ...
    async def test_send_verification_email_org_and_platform(self, mock_request, db, org):
        ...
        with patch(
            ...
            return_value="https://omnilearn.test",
        ), patch(
    ...
    async def test_send_verification_email_platform_uses_platform_url(
        ...
        OMNILEARN_PLATFORM_URL, not the frontend_domain fallback."""
        user = await _make_user(
            db,
            id=23,
        ...
        with patch.dict(
            os.environ, {"OMNILEARN_PLATFORM_URL": "https://www.omnilearn.app"}
        ), patch(
            "src.services.users.email_verification.get_redis_connection",
            return_value=fake_redis,
        ), patch(
            ...
            "src.services.users.email_verification.get_base_url_from_request",
            return_value="https://omnilearn.io",
        ), patch(
            "src.services.users.email_verification.send_email_verification_email",
            return_value=True,
        ) as mock_send:
        ...
        assert mock_send.call_args.kwargs["base_url"] == "https://www.omnilearn.app"
