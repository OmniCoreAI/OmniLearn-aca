...
def test_is_dev_mode_enabled_true():
    with patch("src.services.dev.dev.get_omnilearn_config", return_value=_config(True)):
        assert isDevModeEnabled() is True


def test_is_dev_mode_enabled_false():
    with patch("src.services.dev.dev.get_omnilearn_config", return_value=_config(False)):
