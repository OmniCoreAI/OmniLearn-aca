...
class TestCustomDomainHelpers:
    def test_helper_functions_and_verification_instructions(self):
        ...
        assert is_reserved_domain("omnilearn.io") is True
        ...
        assert info.txt_record_host == "_omnilearn-verification.docs"
        assert info.txt_record_value == "omnilearn-verify=token123"
        assert info.cname_record_host == "docs"
        assert info.cname_record_value == f"test-org.{custom_domains_service.OMNILEARN_DOMAIN}"
    ...
    def test_helper_functions_cover_apex_domain_instructions(self):
        ...
        assert info.txt_record_host == "_omnilearn-verification"
        assert info.cname_record_host == "@"
        assert info.cname_record_value == f"org-slug.{custom_domains_service.OMNILEARN_DOMAIN}"
    ...
    async def test_add_custom_domain_validation_and_duplicate_paths(
        ...
        with pytest.raises(HTTPException) as reserved_exc:
            await add_custom_domain(
                mock_request,
                db,
                custom_domains_service.CustomDomainCreate(domain="https://omnilearn.io/app"),
                org.id,
                admin_user,
            )
...
class TestVerifyDomainDns:
    ...
    async def test_verify_domain_dns_success_and_dns_errors(self, db, org):
        ...
        dns_module, resolver_module = _install_fake_dns_module(
            resolve_return_value=["omnilearn-verify=token-123"],
        )
        ...
        resolver_module.resolve.assert_called_once_with(
            "_omnilearn-verification.docs.example.com",
            "TXT",
        )
        ...
        dns_module, resolver_module = _install_fake_dns_module(
            resolve_return_value=["omnilearn-verify=wrong"],
        )
        with patch.dict(sys.modules, {"dns": dns_module, "dns.resolver": resolver_module}):
    ...
    async def test_verify_domain_dns_dev_mode_and_import_error(self, db, org):
        ...
        with patch.dict(
            os.environ,
            {"OMNILEARN_CUSTOM_DOMAIN_DEV_MODE": "true"},
            clear=False,
        ):
...
class TestAgencySubdomainAutoVerify:
    ...
    def test_is_own_agency_subdomain_matches_only_the_orgs_own_slug_host(self):
        with patch.object(custom_domains_service, "OMNILEARN_DOMAIN", "acme.com"):
    ...
    def test_is_own_agency_subdomain_disabled_for_default_or_dev_apex(self):
        ...
        for base in ("omnilearn.io", "localhost", ""):
            with patch.object(custom_domains_service, "OMNILEARN_DOMAIN", base):
