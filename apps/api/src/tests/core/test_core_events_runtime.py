...
async def test_auto_install_branches(monkeypatch):
    ...
    monkeypatch.setattr(autoinstall, "get_omnilearn_config", lambda: fake_config)
...
async def test_content_and_logs_helpers(monkeypatch, tmp_path):
    ...
    assert basic_config_calls and basic_config_calls[0]["handlers"] == ["file:logs/omnilearn.log", "stream"]
...
async def test_events_startup_shutdown_and_reconcile(monkeypatch):
    ...
    monkeypatch.setattr(core_events, "get_omnilearn_config", lambda: fake_config)
    ...
    assert app.omnilearn_config is fake_config
