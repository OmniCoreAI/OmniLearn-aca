...
async def test_auto_install_triggers_install_when_no_orgs_exist(monkeypatch):
    ...
    monkeypatch.setattr(autoinstall, "get_omnilearn_config", lambda: config)
...
async def test_startup_and_shutdown_app(monkeypatch):
    ...
    monkeypatch.setattr(events, "get_omnilearn_config", lambda: SimpleNamespace(name="cfg"))
    ...
    assert app.omnilearn_config.name == "cfg"
