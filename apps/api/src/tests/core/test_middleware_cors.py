...
class TestGetCorsOriginRegex:
    def test_single_tenancy_pins_to_configured_hosts(self):
        with patch(
            "src.core.middleware.cors.get_omnilearn_config",
    ...
    def test_multi_tenancy_returns_configured_regex(self):
        configured = r"^https?://(.*\.)?omnilearn\.io$"
        with patch(
            "src.core.middleware.cors.get_omnilearn_config",
    ...
    def test_multi_tenancy_falls_back_to_domain_when_regexp_empty(self):
        ...
        with patch(
            "src.core.middleware.cors.get_omnilearn_config",
            return_value=_config("multi", allowed_regexp="", domain="omnilearn.io"),
        ):
            regex = get_cors_origin_regex()
            assert re.fullmatch(regex, "https://omnilearn.io")
            assert re.fullmatch(regex, "https://acme.omnilearn.io")
            assert re.fullmatch(regex, "https://deep.sub.omnilearn.io:8443")
            assert not re.fullmatch(regex, "https://evil.example.org")
