...
class TestBuildPool:
    def test_returns_none_when_conn_string_empty(self):
        ...
        with patch("src.core.redis.get_omnilearn_config", return_value=config):
    ...
    def test_returns_pool_on_success(self):
        ...
        fake_pool = MagicMock()
        with patch("src.core.redis.get_omnilearn_config", return_value=config), patch(
    ...
    def test_returns_none_on_config_exception(self):
        with patch("src.core.redis.get_omnilearn_config", side_effect=RuntimeError("no config")):
            pool = redis_module._build_pool()
        assert pool is None
