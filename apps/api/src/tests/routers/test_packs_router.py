...
class TestInternalPacksRouter:
    async def test_verify_platform_key_errors(self, client):
        old = os.environ.get("OMNILEARN_PLATFORM_API_KEY")
        os.environ.pop("OMNILEARN_PLATFORM_API_KEY", None)
        try:
            ...
            if old is None:
                ...
                os.environ["OMNILEARN_PLATFORM_API_KEY"] = old

        assert response.status_code == 500

        old = os.environ.get("OMNILEARN_PLATFORM_API_KEY")
        os.environ["OMNILEARN_PLATFORM_API_KEY"] = "platform-secret"
        try:
            response = await client.post(
                "/api/v1/internal/packs/1/activate",
