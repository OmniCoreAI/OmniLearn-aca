...
def _mock_config(tenancy: str = "multi"):
    return SimpleNamespace(
        hosting_config=SimpleNamespace(
            tenancy=tenancy,
            domain="omnilearn.test",
            cookie_config=SimpleNamespace(domain=".omnilearn.test"),
        )
    )
...
class TestAuthHelpers:
    def test_get_cookie_domain_for_configured_subdomain(self):
        request = Mock()
        request.headers = {
            "origin": "https://app.omnilearn.test",
            "referer": "",
            "host": "",
        }

        with patch("src.routers.auth.get_omnilearn_config", return_value=_mock_config()):
            assert get_cookie_domain_for_request(request) == ".omnilearn.test"
    ...
    def test_get_cookie_domain_single_tenancy_is_always_host_only(self):
        ...
        for origin in (
            "https://app.omnilearn.test",  # would be subdomain match in multi
            ...
            with patch(
                "src.routers.auth.get_omnilearn_config",
    ...
    def test_set_and_unset_auth_cookies(self):
        ...
        request = Mock()
        request.client = SimpleNamespace(host="127.0.0.1")
        request.headers = {"origin": "https://app.omnilearn.test"}
    ...
    def test_cookie_domain_and_secure_fallback_branches(self):
        ...
        with patch("src.routers.auth.get_omnilearn_config", return_value=_mock_config()), patch(
            "src.routers.auth.isDevModeEnabled",
            return_value=False,
        ):
            assert get_cookie_domain_for_request(empty_request) == ".omnilearn.test"
...
class TestAuthRouter:
    ...
    async def test_oauth_invalid_org_id_returns_400(self, client, db, org):
        with patch("src.routers.auth.get_omnilearn_config"):
            response = await client.post(
                "/api/v1/auth/oauth",
                params={"org_id": 9999},
                json={
    ...
    async def test_oauth_org_id_valid_but_no_invite_clears_org_id(self, client, db, org):
        ...
        with patch("src.routers.auth.get_omnilearn_config", return_value=mock_config), patch(
    ...
    async def test_oauth_org_id_google_verified_checks_invite_and_closes_redis(
        ...
        with patch(
            ...
            "src.routers.auth.get_omnilearn_config", return_value=mock_config
        ), patch(
            "redis.Redis.from_url", return_value=mock_redis
