...
class OmniLearnConfig(BaseModel):
    site_name: str
...
def get_omnilearn_config() -> OmniLearnConfig:
    ...
    # Development Mode
    env_development_mode_str = os.environ.get("OMNILEARN_DEVELOPMENT_MODE", "None")
    ...
    # Sentry config
    env_sentry_dsn = os.environ.get("OMNILEARN_SENTRY_DSN")
    sentry_dsn = env_sentry_dsn or yaml_config.get("general", {}).get("sentry_dsn")

    # Environment (dev or prod)
    omnilearn_env = os.environ.get("OMNILEARN_ENV", "dev")

    # SaaS Mode (enables plan-based gating and usage limits)
    env_saas_mode = os.environ.get("OMNILEARN_SAAS", "None")
    saas_mode = (
    ...
    # Security Config
    env_auth_jwt_secret_key = os.environ.get("OMNILEARN_AUTH_JWT_SECRET_KEY")
    ...
    if not auth_jwt_secret_key:
        raise ValueError(
            "SECURITY ERROR: OMNILEARN_AUTH_JWT_SECRET_KEY must be set. "
            "Generate a secure key with: python -c \"import secrets; print(secrets.token_urlsafe(32))\""
        )
    if len(auth_jwt_secret_key) < 32:
        raise ValueError(
            "SECURITY ERROR: OMNILEARN_AUTH_JWT_SECRET_KEY must be at least 32 characters. "
    ...
    env_site_name = os.environ.get("OMNILEARN_SITE_NAME")
    env_site_description = os.environ.get("OMNILEARN_SITE_DESCRIPTION")
    env_contact_email = os.environ.get("OMNILEARN_CONTACT_EMAIL")
    env_domain = os.environ.get("OMNILEARN_DOMAIN")
    os.environ.get("OMNILEARN_PORT")
    env_ssl = os.environ.get("OMNILEARN_SSL")
    env_port = os.environ.get("OMNILEARN_PORT")
    env_use_default_org = os.environ.get("OMNILEARN_USE_DEFAULT_ORG")
    env_allowed_origins = os.environ.get("OMNILEARN_ALLOWED_ORIGINS")
    env_cookie_domain = os.environ.get("OMNILEARN_COOKIE_DOMAIN")
    env_frontend_domain = os.environ.get("OMNILEARN_FRONTEND_DOMAIN")
    env_tenancy = os.environ.get("OMNILEARN_TENANCY")

    # Allowed origins should be a comma separated string
    if env_allowed_origins:
        env_allowed_origins = env_allowed_origins.split(",")
    env_allowed_regexp = os.environ.get("OMNILEARN_ALLOWED_REGEXP")
    env_self_hosted = os.environ.get("OMNILEARN_SELF_HOSTED")
    env_sql_connection_string = os.environ.get("OMNILEARN_SQL_CONNECTION_STRING")
    ...
    #   - "multi":  slug.{OMNILEARN_DOMAIN} subdomain detection. Requires EE
    ...
    #     `acme.omnilearn.io` to org `acme`).
    #   - Anything else → "single".
    # Existing operators upgrading do not need to set OMNILEARN_TENANCY for
    ...
    if tenancy_explicit:
        ...
        #   1. SaaS mode is on (OMNILEARN_SAAS=true) — SaaS deployments are
        #      always multi-tenant by definition.
        #   2. EE is available AND a shared-subdomain cookie was configured
        #      (OMNILEARN_COOKIE_DOMAIN starts with "."). The dotted cookie
    ...
    if tenancy not in ("multi", "single"):
        raise ValueError(
            f"OMNILEARN_TENANCY must be 'multi' or 'single' (got {tenancy_raw!r})."
        )
    ...
    # the operator opts in with OMNILEARN_COOKIE_DOMAIN_ALLOW_BROAD=true.
    ...
    if (
        ...
        allow_broad = os.environ.get(
            "OMNILEARN_COOKIE_DOMAIN_ALLOW_BROAD", ""
        ).lower() in ("true", "1", "yes")
        if "." not in tenant_eTLD:
            raise ValueError(
                f"SECURITY ERROR: OMNILEARN_COOKIE_DOMAIN={cookies_domain!r} is "
        ...
        if not allow_broad:
            ...
            _cfg_log.getLogger(__name__).warning(
                "OMNILEARN_COOKIE_DOMAIN is set to the broad parent %r. "
                "Every subdomain will share the session cookie. If that is "
                "intentional, set OMNILEARN_COOKIE_DOMAIN_ALLOW_BROAD=true to "
    ...
    env_content_delivery_type = os.environ.get("OMNILEARN_CONTENT_DELIVERY_TYPE")
    content_delivery_type: str = env_content_delivery_type or (
        (yaml_config.get("hosting_config", {}).get("content_delivery", {}).get("type"))
        or "filesystem"
    )  # default to filesystem

    env_bucket_name = os.environ.get("OMNILEARN_S3_API_BUCKET_NAME")
    env_endpoint_url = os.environ.get("OMNILEARN_S3_API_ENDPOINT_URL")
    bucket_name = (
    ...
    env_gemini_api_key = os.environ.get("OMNILEARN_GEMINI_API_KEY")
    env_is_ai_enabled_str = os.environ.get("OMNILEARN_IS_AI_ENABLED")
    ...
    ai_provider = os.environ.get("OMNILEARN_AI_PROVIDER") or yaml_ai_config.get("provider")
    ai_api_key = os.environ.get("OMNILEARN_AI_API_KEY") or yaml_ai_config.get("api_key")
    ai_base_url = os.environ.get("OMNILEARN_AI_BASE_URL") or yaml_ai_config.get("base_url")
    ai_model_fast = os.environ.get("OMNILEARN_AI_MODEL_FAST") or yaml_ai_config.get("model_fast")
    ai_model_standard = os.environ.get("OMNILEARN_AI_MODEL_STANDARD") or yaml_ai_config.get("model_standard")
    ai_model_pro = os.environ.get("OMNILEARN_AI_MODEL_PRO") or yaml_ai_config.get("model_pro")
    ai_embedding_provider = os.environ.get("OMNILEARN_AI_EMBEDDING_PROVIDER") or yaml_ai_config.get("embedding_provider")
    ai_embedding_model = os.environ.get("OMNILEARN_AI_EMBEDDING_MODEL") or yaml_ai_config.get("embedding_model")
    _ai_embedding_dims = os.environ.get("OMNILEARN_AI_EMBEDDING_DIMENSIONS") or yaml_ai_config.get("embedding_dimensions")
    ...
    # Redis config
    env_redis_connection_string = os.environ.get("OMNILEARN_REDIS_CONNECTION_STRING")
    redis_connection_string = env_redis_connection_string or yaml_config.get(
        "redis_config", {}
    ).get("redis_connection_string")

    # Mailing config
    env_email_provider = os.environ.get("OMNILEARN_EMAIL_PROVIDER")
    env_resend_api_key = os.environ.get("OMNILEARN_RESEND_API_KEY")
    env_system_email_address = os.environ.get("OMNILEARN_SYSTEM_EMAIL_ADDRESS")
    env_smtp_host = os.environ.get("OMNILEARN_SMTP_HOST")
    env_smtp_port = os.environ.get("OMNILEARN_SMTP_PORT")
    env_smtp_username = os.environ.get("OMNILEARN_SMTP_USERNAME")
    env_smtp_password = os.environ.get("OMNILEARN_SMTP_PASSWORD")
    env_smtp_use_tls = os.environ.get("OMNILEARN_SMTP_USE_TLS")
    ...
    env_tinybird_api_url = os.environ.get("OMNILEARN_TINYBIRD_API_URL")
    env_tinybird_ingest_token = os.environ.get("OMNILEARN_TINYBIRD_INGEST_TOKEN")
    env_tinybird_read_token = os.environ.get("OMNILEARN_TINYBIRD_READ_TOKEN")
    ...
    env_judge0_api_url = os.environ.get("OMNILEARN_JUDGE0_API_URL")
    env_judge0_client_id = os.environ.get("OMNILEARN_JUDGE0_CLIENT_ID")
    env_judge0_client_secret = os.environ.get("OMNILEARN_JUDGE0_CLIENT_SECRET")
    ...
    # Payments config
    env_stripe_secret_key = os.environ.get("OMNILEARN_STRIPE_SECRET_KEY")
    env_stripe_publishable_key = os.environ.get("OMNILEARN_STRIPE_PUBLISHABLE_KEY")
    env_stripe_webhook_standard_secret = os.environ.get("OMNILEARN_STRIPE_WEBHOOK_STANDARD_SECRET")
    env_stripe_webhook_connect_secret = os.environ.get("OMNILEARN_STRIPE_WEBHOOK_CONNECT_SECRET")
    env_stripe_client_id = os.environ.get("OMNILEARN_STRIPE_CLIENT_ID")
    ...
    if not _TESTING_TENANCY:
        ...
        if tenancy == "multi":
            ...
            # parent for browsers in production, so `OMNILEARN_DOMAIN=localhost`
            # is rejected here.
            try:
            ...
            if not (ee_available or saas_mode):
                raise ValueError(
                    "OMNILEARN_TENANCY=multi requires the Enterprise Edition "
                    "or OMNILEARN_SAAS=true. Either install the `ee/` folder, "
                    "unset OMNILEARN_DISABLE_EE, or use OMNILEARN_TENANCY=single."
                )
            if not domain or "localhost" in str(domain).lower():
                raise ValueError(
                    "OMNILEARN_TENANCY=multi requires OMNILEARN_DOMAIN to be set "
                    "to a routable parent domain (e.g. 'omnilearn.io'). "
                    f"Got domain={domain!r}."
                )

        if tenancy == "single":
            # In single mode the cookie domain is always host-only — operators
            # who set OMNILEARN_COOKIE_DOMAIN expecting cross-subdomain auth
            # will be surprised. Warn loudly.
            if env_cookie_domain:
                _t_logger.warning(
                    "OMNILEARN_COOKIE_DOMAIN=%r is set but OMNILEARN_TENANCY=single "
                    "ignores it (cookies are host-only in single mode).",
                    env_cookie_domain,
                )

        # Deprecations: both flags are subsumed by OMNILEARN_TENANCY.
        if env_use_default_org is not None:
            _t_logger.warning(
                "OMNILEARN_USE_DEFAULT_ORG is deprecated; tenancy mode "
                "(OMNILEARN_TENANCY=%s) now controls default-org behavior.",
                tenancy,
            )
        if env_self_hosted is not None:
            _t_logger.warning(
                "OMNILEARN_SELF_HOSTED is deprecated; use OMNILEARN_TENANCY=single "
    ...
    if not development_mode and saas_mode:
        ...
        if not os.environ.get("OMNILEARN_PLATFORM_API_KEY"):
            _log.warning(
                "OMNILEARN_PLATFORM_API_KEY is not set. Platform endpoints "
                "(packs internal_router) will reject every request."
            )

    # Create OmniLearnConfig object
    config = OmniLearnConfig(
        site_name=site_name,
