...
Monitoring your self-hosted OmniLearn instance helps you catch issues early and ensure a reliable experience for your users.
...
```bash
omnilearn status    # quick container status for each service
omnilearn health    # PostgreSQL, Redis, HTTP health endpoint, disk, resources
omnilearn doctor    # deep diagnostics (Docker, ports, DNS, env validation, logs)
```
...
- Run `omnilearn doctor` regularly to verify all services are running and healthy
...
- OmniLearn requires at least 2 GB of RAM
...
- **Uptime monitoring** -- use an external service to check that your OmniLearn instance is accessible (e.g., monitor `http://your-domain/api/v1/health`)
