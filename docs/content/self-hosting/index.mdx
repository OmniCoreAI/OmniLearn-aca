...
# Self-Hosting OmniLearn

OmniLearn can be self-hosted on your own infrastructure using the **OmniLearn CLI**. The CLI handles everything from initial setup to ongoing maintenance, including Docker Compose generation, environment file management, service orchestration, backups, and health checks.
...
OmniLearn will be available at **http://localhost** (or your configured domain). Log in with the admin credentials you provided during setup.
...
npx omnilearn config     # View current configuration
npx omnilearn backup     # Backup or restore the database
