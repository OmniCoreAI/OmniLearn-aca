...
- **Infrastructure** (Docker): PostgreSQL with pgvector + Redis run in containers via `.omnilearn/docker-compose.dev.yml`
...
```bash
docker compose -f .omnilearn/docker-compose.dev.yml -p omnilearn-dev down
```
