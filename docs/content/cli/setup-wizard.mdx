...
The setup wizard (`npx omnilearn@latest setup`) is an interactive 6-step process that generates a complete Docker Compose deployment. You can go back and edit any step before confirming.
...
- **HTTPS options** (non-localhost only):
  ...
  - **Manual SSL** — You handle the reverse proxy or Cloudflare. OmniLearn runs on HTTP internally.
...
| --- | --- | --- |
| `omnilearn-app-{id}` | `ghcr.io/omnilearn/app:latest` (or `:dev`) | Next.js + FastAPI application |
| `omnilearn-db-{id}` | `pgvector/pgvector:pg16` | PostgreSQL database |
| `omnilearn-redis-{id}` | `redis:7.2.3-alpine` | Redis cache |
| `omnilearn-caddy-{id}` | `caddy:2-alpine` | Reverse proxy with auto-SSL |
| `omnilearn-nginx-{id}` | `nginx:alpine` | Reverse proxy (no auto-SSL) |
...
- **Stable** — Recommended for production (pulls `ghcr.io/omnilearn/app:latest`)
- **Dev** — Bleeding edge, latest features (pulls `ghcr.io/omnilearn/app:dev`)

To pin to a specific image tag after setup, use `omnilearn update --version <tag>`.
