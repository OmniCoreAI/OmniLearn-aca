...
Full reference for all OmniLearn CLI commands.
...
Show the live status of each OmniLearn container.

```bash
npx omnilearn status
```
...
```bash
npx omnilearn config
```

## `update`

Update OmniLearn to the latest release or a specific version. See [Updates](/self-hosting/maintenance/updates) for details.
...
npx omnilearn update --to 1.4.2
# or using the short form:
npx omnilearn update -v 1.4.2
```

| Flag | Description |
|---|---|
| `-v, --to <version>` | Pull a specific image tag from `ghcr.io/omnilearn/app` (e.g. `1.4.2`, `v1.4.2`). Defaults to `latest`. |
...
If the migration step fails, the CLI prints the rollback command and exits with a non-zero code. Your pre-upgrade backup is in `{installDir}/backups/`. To roll back: restore the backup, revert the image tag in `docker-compose.yml`, and run `npx omnilearn start`.
</Callout>
...
Backups are saved to `{installDir}/backups/omnilearn-backup-{timestamp}.tar.gz`.
...
```bash
npx omnilearn restore <archive.tar.gz>
```

Alternatively:

```bash
npx omnilearn backup --restore <archive.tar.gz>
```

<Callout type="warning">
Restore only works with local (Docker) databases, not external Postgres connections. Restart services after restoring: `npx omnilearn stop && npx omnilearn start`.
</Callout>
...
```bash
npx omnilearn deployments
```

Features:
- List all OmniLearn deployments with container status
- Scale memory limits for `omnilearn-app`, `db`, and `redis`
...
Run `npx omnilearn doctor` — it scans logs and surfaces the root cause. Common causes:
...
If `npx omnilearn update` reports a migration failure:

```bash
# 1. Restore your pre-upgrade backup
npx omnilearn restore backups/db-pre-upgrade-*.sql.gz

# 2. Revert the image tag in docker-compose.yml to the previous version
# (open the file and change image: ghcr.io/omnilearn/app:<new> back to <old>)

# 3. Restart on the old image
npx omnilearn start
```

Then check the [OmniLearn releases page](https://github.com/omnilearn/omnilearn/releases) and the [GitHub issues](https://github.com/omnilearn/omnilearn/issues) for your version before retrying.

### Database unreachable after update

The `omnilearn-db` container may need a moment after startup. Wait 20-30 seconds and run `npx omnilearn health`. If health checks keep failing, run `npx omnilearn logs` to see the Postgres logs.
...
docker compose exec db psql -U omnilearn -c "DROP DATABASE omnilearn; CREATE DATABASE omnilearn;"
npx omnilearn restore <backup.tar.gz>
```
