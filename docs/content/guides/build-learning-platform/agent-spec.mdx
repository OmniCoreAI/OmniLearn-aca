---
title: Agent Spec
description: A self-contained, copy-paste specification a coding agent can follow to build a headless OmniLearn learning platform with Next.js, including the exact API contract and acceptance criteria.
---
...
Copy everything in the block below, fill in the **Configuration** values at the top, and paste it into your coding agent (see [Let an Agent Build It](/guides/build-learning-platform/with-an-agent) for the workflow). It's written *for the model* and contains the real OmniLearn API contract, so the agent builds against the API that actually exists.
...
````markdown
# SPEC: Build a headless learning platform on the OmniLearn API

## Configuration (fill these in before starting)
- API_BASE = http://localhost:1338/api/v1     # your OmniLearn API base URL
...
that reads courses and content from the OmniLearn REST API and supports user
...
## Hard rules
- OmniLearn access/refresh tokens MUST NEVER reach the browser. Do all
...
- GET  /auth/refresh
    Reads the refresh token ONLY from the `OL_refresh` cookie — NOT a header or
    body. To refresh from the server, send header: Cookie: OL_refresh={value}.
...
- src/lib/omnilearn.ts   : API_BASE/ORG_SLUG consts, `lhFetch` (public reads,
...
- src/lib/session.ts      : httpOnly-cookie session (get/set/clear) + `authedFetch`
                            ...
                            sent as a Cookie: OL_refresh header), persisting the
...
## Environment
Create .env.local:
  NEXT_PUBLIC_OMNILEARN_API_URL=<API_BASE>
  NEXT_PUBLIC_OMNILEARN_ORG_SLUG=<ORG_SLUG>

## Build order
1. Scaffold: npx create-next-app@latest . --typescript --app --tailwind --eslint --src-dir --use-npm
2. Stage 1: lib/omnilearn.ts -> catalogue -> course page -> activity viewer.
