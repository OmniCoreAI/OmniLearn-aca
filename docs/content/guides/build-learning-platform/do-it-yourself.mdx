...
description: A complete, tested, step-by-step guide to building a headless learning platform with Next.js and the OmniLearn API — public catalogue first, then authentication, enrollment, and progress tracking.
---
...
This is the full build. By the end you'll have a Next.js app that browses your courses, plays their content, signs learners in, enrolls them, and tracks progress — all on top of the OmniLearn API.
...
<Callout type="info">
**Prerequisites** (from the [overview](/guides/build-learning-platform)): Node 18+, a running OmniLearn instance at `http://localhost:1338`, your **org slug**, and at least one **public, published course** with content. Everything reads the API base from one env var, so adjust it if your instance is elsewhere.
</Callout>
...
NEXT_PUBLIC_OMNILEARN_API_URL=http://localhost:1338/api/v1
NEXT_PUBLIC_OMNILEARN_ORG_SLUG=your-org-slug
```
...
Every call to OmniLearn goes through one small helper. Create `src/lib/omnilearn.ts`. This mirrors the pattern the official frontend uses in `apps/web/services/utils/ts/requests.ts` — a thin `fetch` wrapper that surfaces the API's `{ "detail": "..." }` error shape.

```ts filename="src/lib/omnilearn.ts"
export const API_URL = process.env.NEXT_PUBLIC_OMNILEARN_API_URL!
export const ORG_SLUG = process.env.NEXT_PUBLIC_OMNILEARN_ORG_SLUG!

/**
 * Fetch JSON from the OmniLearn API.
...
export async function lhFetch<T = any>(
  ...
  if (!res.ok) {
    ...
    throw new Error(`OmniLearn API ${res.status}: ${detail}`)
  }
  return res.json()
}
...
```ts filename="src/lib/omnilearn.ts (continued)"
export type Course = {
  course_uuid: string
  name: string
  description: string
...
import { lhFetch, courseThumbnail, ORG_SLUG, type Course } from '@/lib/omnilearn'
...
Open [http://localhost:3000](http://localhost:3000) and you'll see your courses. (If OmniLearn is also on `:3000`, run the API on its usual port and Next will pick `:3001` — or set `PORT`.)
...
import Link from 'next/link'
import { lhFetch, type CourseMeta } from '@/lib/omnilearn'
...
import { lhFetch, activityMedia, ORG_SLUG, type Activity } from '@/lib/omnilearn'
...
Now we add accounts. The golden rule: **OmniLearn tokens never reach the browser.** Your Next.js app logs in on the server, stores the tokens in its *own* httpOnly cookie, and attaches them when calling the API. This is the Backend-for-Frontend (BFF) pattern.

<AuthFlowDiagram />

<Callout type="warning">
**The refresh gotcha.** `POST /api/v1/auth/login` returns `access_token` **and** `refresh_token` in the JSON body. But `GET /api/v1/auth/refresh` reads the refresh token **only from the `OL_refresh` cookie** — not a header and not the body. So when your server refreshes, it must send the stored refresh token *as the `OL_refresh` cookie*. We handle this in `lib/session.ts` below.
</Callout>

## Step 6 — A server-side session

Create `src/lib/session.ts` to read and write the session cookie. It stores the OmniLearn tokens, encrypted-at-rest concerns aside, in an httpOnly cookie that the browser can't read from JavaScript.
...
import { API_URL } from '@/lib/omnilearn'
...
import { NextRequest, NextResponse } from 'next/server'
import { API_URL, lhFetch, ORG_SLUG } from '@/lib/omnilearn'
...
export async function authedFetch<T = any>(
  ...
  if (res.status === 401) {
    // /auth/refresh reads the refresh token from the OL_refresh *cookie*,
    // so send it as one (not as a header or body).
    const refreshed = await fetch(`${API_URL}/auth/refresh`, {
      headers: { Cookie: `OL_refresh=${session.refresh_token}` },
    })
    if (refreshed.ok) {
  ...
  if (!res.ok) throw new Error(`OmniLearn API ${res.status}`)
  return res.json()
}
```
...
- **Compare with the reference** — see [`omnilearn/headless-examples`](https://github.com/omnilearn/headless-examples) for a complete project.
