...
OmniLearn supports two tenancy modes, selected by a single environment variable: `OMNILEARN_TENANCY`.
...
```
OMNILEARN_TENANCY=multi   # requires EE (or SaaS) + OMNILEARN_DOMAIN
OMNILEARN_TENANCY=single  # default for OSS / dev / VPS self-host
# unset                    # inferred — see "Default inference" below
```

The backend validates at boot:
- `multi` requires `is_ee_available()` OR `OMNILEARN_SAAS=true`, AND `OMNILEARN_DOMAIN` set to a routable parent domain (not `localhost`). Mis-configuration aborts startup with a clear error.
- `single` ignores `OMNILEARN_DOMAIN` and `OMNILEARN_COOKIE_DOMAIN`. If you set them, you'll see a deprecation warning at boot — they are no longer authoritative.

### Default inference (when `OMNILEARN_TENANCY` is unset)
...
|----------------------------------------------------------------------------------------------|------------------|
| `OMNILEARN_SAAS=true` + real (non-localhost) domain + not self-hosted/dev                   | `multi`          |
| EE folder present + `OMNILEARN_COOKIE_DOMAIN` starts with `.` + real domain + not self/dev  | `multi`          |
| Anything else                                                                                | `single`         |

The strong signal for "I want subdomain tenancy" is a dotted cookie domain (`OMNILEARN_COOKIE_DOMAIN=.foo.com`) — operators only set that when they want subdomains to share auth. This means an EE deployment on a single VPS domain (no dotted cookie domain) defaults to `single`, which is the right behavior for the self-host-EE case (one org, EE features locally available).

**Recommended:** set `OMNILEARN_TENANCY` explicitly in production. Inference is a backward-compat convenience, not a long-term contract.

## Compatibility with deployment modes (oss / ee / saas)

`OMNILEARN_TENANCY` is orthogonal to the existing **deployment mode** (`oss`, `ee`, `saas`) which is set by the presence of the `ee/` folder and `OMNILEARN_SAAS`. The matrix:
...
Existing EE/SaaS deployments that haven't set `OMNILEARN_TENANCY` continue to behave as before because the inference reads their existing config (`OMNILEARN_SAAS`, `OMNILEARN_COOKIE_DOMAIN`, `OMNILEARN_DOMAIN`) and picks `multi` when those signal subdomain intent.

## Mode 1 — Multi (EE)

Required env:
- `OMNILEARN_TENANCY=multi`
- `OMNILEARN_DOMAIN=your-domain.tld` (the apex; e.g. `omnilearn.io`)
- `OMNILEARN_COOKIE_DOMAIN=.your-domain.tld` (so subdomains share the session)
- `ee/` folder present (or unset `OMNILEARN_DISABLE_EE`)
...
Required env:
- `OMNILEARN_TENANCY=single` (or just leave unset; this is the default)
...
All OmniLearn cookies use the `OL_` prefix.

| Cookie                | Set by         | Purpose                                                                                       |
|-----------------------|----------------|-----------------------------------------------------------------------------------------------|
| `OL_access`           | API            | JWT access token (httpOnly, 8-hour TTL).                                                      |
| `OL_refresh`          | API            | JWT refresh token (httpOnly, 30-day rotating).                                                |
| `OL_org`              | middleware     | Canonical org slug for the current request. **Source of truth.**                              |
| `OL_tenancy`          | middleware     | `multi` or `single`. Source of truth for `getTenancy()` on the frontend.                      |
| `OL_default_org`      | middleware     | Default org slug from `instance/info`.                                                        |
| `OL_frontend_domain`  | middleware     | Configured frontend domain (for cookie-domain math).                                          |
| `OL_top_domain`       | middleware     | Configured top domain (apex without port).                                                    |
| `OL_mode`             | middleware     | Deployment mode: `oss`, `ee`, or `saas`.                                                      |
| `OL_custom_domain`    | middleware     | Set in multi mode when the request came in on a verified per-org custom domain.               |
| `OL_session`          | platform API   | Non-httpOnly flag indicating an active session, read by client-side gating.                   |
| `OL_region`           | platform mw    | `eur` or `usd`, derived from Vercel geolocation, used for Stripe pricing.                     |
| `OL_last_org`         | localStorage   | Last visited org slug (UX hint, not security-relevant).                                       |
| `OL_oauth_state`      | client         | OAuth CSRF state (5-min TTL).                                                                 |
| `OL_oauth_orgslug`    | client         | Org slug context preserved across the OAuth round-trip.                                       |
| `OL_oauth_org_id`     | client         | Org ID context preserved across the OAuth round-trip.                                         |
...
| Header                  | Purpose                                                                       |
|-------------------------|-------------------------------------------------------------------------------|
| `x-lh-tenancy`          | `multi` or `single` — same as `OL_tenancy` cookie but available to RSC on the first cold load |
...
- **`OL_access` / `OL_refresh`**: signed JWTs (HS256, server-side secret). Tampering invalidates the signature → API rejects. Also bound to `password_changed_at` and a Redis revocation list, so stale-but-valid tokens get rejected after logout/password change.
- **`OL_tenancy`, `OL_default_org`, `OL_frontend_domain`, `OL_top_domain`, `OL_mode`, `OL_custom_domain`**: UX hints only. The middleware fetches authoritative tenancy/mode from the backend (`/instance/info`, server-side config). Editing these cookies only affects the rendered UI in that user's browser; they cannot redirect routing to a different deployment shape.
- **`OL_org`**: also a hint. The proxy may rewrite to `/orgs/{tampered-slug}/...`, but every API endpoint re-checks `require_org_membership(user_id, org_id)`. Worst case: the tamperer sees a broken UI shell with no data.
- **`OL_session`**: non-httpOnly boolean. Setting it to `1` without a valid JWT just triggers the "redirect logged-in users away from /login" branch — the dashboard then fails real auth and redirects back. Annoyance, not breach.
- **`OL_region`**: changes which currency Stripe shows. **Stripe enforces server-side prices at checkout** — the user manipulates display, not the charge.
...
The cookie naming convention changed in this release. **All cookies now use the `OL_` prefix and legacy names are no longer read.** Old cookie names (`access_token_cookie`, `refresh_token_cookie`, `omnilearn_orgslug`, `omnilearn_tenancy`, `omnilearn_multi_org`, `omnilearn_current_orgslug`, `omnilearn_has_session`, `lh_region`, etc.) are dropped entirely — there is no read-fallback window.

**Impact on rollout:** existing logged-in users will be signed out when this version deploys, because their browsers hold cookies the new code does not recognize. They will re-authenticate once and the new `OL_*` cookies will be set. There is no data loss; only an interactive sign-in.
...
|------------|----------------------------------------------------------------------------|----------------------------------------------|
| `multi`    | Configured via `OMNILEARN_ALLOWED_REGEXP` (matches apex + subdomains)     | Strict allowlist for known multi-tenant domain |
...
```json
{
  ...
  "frontend_domain": "omnilearn.io",
  "top_domain": "omnilearn.io"
}
```
...
`_is_allowed_base_url` validates the URL against an allowlist before returning it. In `single` mode the operator's host is authoritative — any well-formed http(s) URL is accepted. In `multi` mode, the URL must match `OMNILEARN_ALLOWED_ORIGINS` / `OMNILEARN_ALLOWED_REGEXP` / `OMNILEARN_PLATFORM_URL`.

## EE / OSS code split

The OSS frontend never imports subdomain or custom-domain logic directly. All of that lives under `omnilearn/apps/web/ee/services/tenancy/`:
...
The OSS `proxy.ts` and `services/org/orgResolution.ts` reach these via dynamic `import()` wrapped in try/catch. If EE is unavailable (folder removed at deploy time, or `OMNILEARN_DISABLE_EE=1`) the resolver falls back to the default org and logs a warning.
...
```sh
OMNILEARN_TENANCY=multi
OMNILEARN_DOMAIN=lh.test
OMNILEARN_COOKIE_DOMAIN=.lh.test
```
Add hosts entries:
```
...
```sh
OMNILEARN_TENANCY=single
# OMNILEARN_DOMAIN unset
```
...
```sh
OMNILEARN_DISABLE_EE=1
OMNILEARN_TENANCY=multi   # should fail at boot
OMNILEARN_TENANCY=single  # should boot cleanly
```
With `single`, no EE imports are attempted at runtime.

## Upgrading from versions before `OMNILEARN_TENANCY` existed

Prior versions decided multi-vs-single behavior implicitly from `is_multi_org_allowed()` (deployment mode in `ee`/`saas`), `self_hosted`, `development_mode`, and `OMNILEARN_USE_DEFAULT_ORG`. After the introduction of `OMNILEARN_TENANCY` these flags are deprecated and the new env var is the source of truth.
...
1. Check the boot log for any deprecation warnings about `OMNILEARN_USE_DEFAULT_ORG`, `OMNILEARN_SELF_HOSTED`, or `OMNILEARN_COOKIE_DOMAIN` being set in single mode.
2. Hit `GET /api/v1/instance/info` and confirm `tenancy` matches your expectation.
3. Set `OMNILEARN_TENANCY` explicitly in your env config to lock the behavior — inference is a backward-compat convenience, not a long-term contract.
...
2. **`multi` mode** requires:
   - EE folder included in the build (`docker build --build-arg OMNILEARN_PUBLIC=false`)
   ...
   - Backend env: `OMNILEARN_TENANCY=multi`, `OMNILEARN_DOMAIN`, `OMNILEARN_COOKIE_DOMAIN`, `OMNILEARN_ALLOWED_REGEXP`
3. **`single` mode** on a VPS:
   - Point any domain at the server. No env config beyond `OMNILEARN_TENANCY=single` is needed.
...
Backend:
- `omnilearn/apps/api/config/config.py` — `OMNILEARN_TENANCY` env, validation
- `omnilearn/apps/api/src/routers/instance.py` — `instance/info` endpoint
- `omnilearn/apps/api/src/routers/auth.py` — `get_cookie_domain_for_request`
- `omnilearn/apps/api/src/services/email/utils.py` — `get_org_signup_base_url`
- `omnilearn/apps/api/app.py` — CORS branching
- `omnilearn/apps/api/src/core/ee_hooks.py` — `is_multi_org_allowed`, EE detection

Frontend:
- `omnilearn/apps/web/proxy.ts` — middleware (resolveTenant)
- `omnilearn/apps/web/ee/services/tenancy/core.ts` — pure resolver logic
- `omnilearn/apps/web/ee/services/tenancy/resolveMulti.middleware.ts` — Edge entry
- `omnilearn/apps/web/ee/services/tenancy/resolveMulti.server.ts` — Node entry
- `omnilearn/apps/web/services/org/orgResolution.ts` — server-side org resolution
- `omnilearn/apps/web/services/auth/cookies.ts` — cookie options
- `omnilearn/apps/web/services/config/config.ts` — `getTenancy`, `getUriWithOrg`
