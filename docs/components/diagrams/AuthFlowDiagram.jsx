...
// holds the OmniLearn tokens in its own httpOnly cookie.
const nodes = [
...
export default function AuthFlowDiagram() {
  return (
    <FlowDiagram
      nodes={nodes}
      caption="The browser only ever holds your app's own session cookie — never the OmniLearn tokens. To refresh, your server re-sends the stored refresh token to GET /auth/refresh as a Cookie: OL_refresh=… header (the endpoint reads it only from that cookie)."
    />
  )
}
